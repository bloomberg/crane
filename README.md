...
A curated selection of C++ files generated by Crane (accompanied with their source Rocq programs) is available to view in the `tests` directory. The tests are organized thematically, with `tests/basics` containing traditional, functional Rocq programs, and `tests/monadic` containing effectful examples.
