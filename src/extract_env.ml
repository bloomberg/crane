...
let print_structure_to_file (fn, si, mo) dry struc =
  ...
  (* Template parameters with function constraints use std::is_invocable_v
     (<type_traits>).  Inline clone code generated by gen_clone_field_expr
     uses std::unique_ptr (<memory>) and std::optional (<optional>) via
     if constexpr / requires expressions.  BDE brings in <memory> and
     <type_traits> via bsl_memory.h and bsl_type_traits.h, but still needs
     <optional> for std::nullopt. *)
