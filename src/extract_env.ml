...
let print_structure_to_file (fn, si, mo) dry struc =
  ...
  set_phase Pre;
  (* Capture the pre-phase struct output for the emptiness check below. *)
  ...
  let skip_file =
    d.skip_empty_files && (not dry) && fn <> None
  ...
  let cout = if dry || skip_file then None else Option.map open_out fn in
  let ft =
    if skip_file then Format.make_formatter (fun _ _ _ -> ()) (fun () -> ())
