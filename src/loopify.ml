...
and collect_stmts check ~in_visitor stmts =
  (* Detect void tail-call pattern: [Sexpr call; Sreturn None] at the end
     of a statement list.  In C++, a void function can end with
     [f(); return;] which is semantically [return f();] — the call is in
     tail position.  Generated by cofix_wrap and gen_stmts for void
     cofixpoints / void-returning recursive functions. *)
  let rec go = function
...
let transform_tail ?(param_inits = []) check _pp_type params ret_ty body =
  ...
  let rec strip_empty_blocks = function
    | Sblock stmts :: rest ->
      ...
      ( match stmts' with
      | [] -> strip_empty_blocks rest
      | _ -> Sblock stmts' :: strip_empty_blocks rest )
    | s :: rest -> s :: strip_empty_blocks rest
    | [] -> []
  in
  let body'' = strip_empty_blocks body'' in
  let continue_decl =
