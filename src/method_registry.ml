...
(** Method registry: pre-scans the full extraction structure to identify which
    functions should become methods on eponymous inductive types.

    This module is the first pass in Crane's multi-pass extraction pipeline. It
    runs before any C++ code is emitted, building a table that maps function
    GlobRef.t values to their method_info records.

    The overall flow is: 1. [create] iterates every module in the ml_structure.
    2. For each module, it finds the eponymous inductive (if any) — the
    inductive whose lowercase name matches the module name. 3. For each function
    in that module (and wrapper modules), it checks whether the function's type
    has the eponymous type as an argument. 4. It verifies the function body is
    safe (no bare [return this]). 5. After all methods are found,
    [compute_returns_any] checks which methods have return types that need to be
    erased to [std::any].
...
(** The method registry: maps function refs to method info and stores
    candidates. *)
type t = {
  methods : (GlobRef.t, method_info) Hashtbl.t;
  (* Reverse index: inductive GlobRef -> list of method candidates. Populated
     during pre-scan so cpp.ml doesn't need to re-collect. *)
...
(** Check if a function body is safe to turn into a method.
    ...
    Background: when a function becomes a C++ method, its first argument is
    received via the [this] pointer (a raw pointer) rather than as a
    [std::shared_ptr]. If the function's body directly returns that first
    argument (a bare [MLrel] in tail position), the generated C++ would produce
    [return this;] — but [this] is a raw pointer and cannot be implicitly
    ...
    passing to other methods is fine — those translate to [this->field],
    [this->v()], and [this->method()] respectively, none of which require
    converting [this] back to [shared_ptr].

    The check works by: 1. Stripping outer lambdas to find the inner body and
    computing the de Bruijn index of the first argument. 2. Recursing through
    tail positions (match branches, let bodies, fixpoint bodies, MLmagic
    wrappers) to see if any tail position is a bare [MLrel] pointing at the
    first argument. 3. Special-casing top-level matches on the first argument
...
let body_safe_for_method body =
  ...
  (* Check if [MLrel target_db] appears as a direct return value in the AST. A
     ...
     [depth] tracks additional binders introduced inside the body (e.g. match
     branch bindings, let bindings) that shift the de Bruijn index of the target
     argument. *)
  ...
  | MLcase (_, MLrel scrut_idx, branches) when scrut_idx = target_db ->
    (* Case 1: Top-level match on first arg (e.g. [match xs with ...]). Only
       check branches, not the scrutinee — the scrutinee is the first argument
       but is used for dispatch, not directly returned. *)
    not
      (Array.exists
         (fun (ids, _, _, branch_body) ->
           let n_bindings = List.length ids in
  ...
  | MLfix (fix_idx, _, funs, _) ->
    (* Case 2: Top-level fixpoint (recursive function). Check the body of the
       ...
       binders (the mutually recursive functions). *)
...
(** Find the eponymous inductive in a module's declarations.
    ...
    An inductive is "eponymous" when its lowercase name matches the module name
    (also lowercased). For example, module [List] has eponymous inductive
    [list], module [Nat] has eponymous inductive [nat].

    Type-class inductives are excluded because they become C++ concepts rather
    than structs, so attaching methods to them does not apply.

    Only searches [SEdecl (Dind ...)] entries at the current level; does not
...
let find_eponymous_inductive module_name_str decls =
  ...
  List.find_map
    (fun (_l, se) ->
      match se with
      | SEdecl (Dind (kn, ind)) ->
        (* A single Dind may have multiple packets (mutual inductives). Check
           each packet's name against the module name. *)
        let rec find_in_packets i =
          if i >= Array.length ind.ind_packets then
            ...
            if String.lowercase_ascii ind_name = lowercase_module then
              ...
              with
              | TypeClass _ ->
                None (* Type classes become concepts, not methods *)
...
(** Find the position of the first argument matching an eponymous type.
    ...
    Walks the curried arrow type [T1 -> T2 -> ... -> Tn -> R], checking each
    [Ti] to see if it is [Tglob(epon_ref, tvar_args, _)].

    Returns [Some (pos, ind_tvar_positions)] where:
    - [pos] is the 0-based argument index that will become [this].
    - [ind_tvar_positions] lists which of the function's type variables are used
      as type parameters of the inductive at that argument position. These are
      already deducible from the receiver type and can be omitted from the
    ...
    - [tvar_args = [Tvar 1]], so [ind_tvar_positions = [0]] (Tvar 1 maps to
      0-based index 0 in the type parameter list). *)
...
(** Register all eligible methods for a given eponymous type from a list of
    ...
    Module matching:
    - [~cross_module:false] (default): only functions in the same [ModPath] as
      the eponymous type are considered.
    - [~wrapper_module_name:Some name]: also allows functions from modules whose
      last path component matches [name] (case-insensitive, prefix match). This
      handles "wrapper modules" where e.g. module [ListAux] contains additional
      functions for [list].
    - [~cross_module:true]: allows any module. Used when registering methods
      from parent-level declarations that reference the eponymous type.
...
(** Pre-register all methods from the entire structure before code generation.
    This ensures that cross-module method calls (like [List.app] called from a
    different module) are recognized correctly during code generation.

    The scan has two levels of behavior controlled by [~at_top_level]:

    {b Top level} ([at_top_level = true]): For inductives at the actual top
    level of the extraction (e.g. stdlib's [list] which lives at file scope, not
    inside a module), sibling functions in the same file-level declarations are
    checked as method candidates. Additionally, functions in parent-level
    declarations that match a wrapper module naming pattern are also registered.

    {b Inside modules} ([at_top_level = false]): For inductives inside a
    [SEmodule], the standard eponymous type detection applies: the module name
    is compared against inductive names, and functions within that module are
    checked.

    Both levels perform early enum detection (needed so that
    [register_methods_for_epon] can skip enum types), then recurse into
    sub-modules.

    [parent_decls] accumulates declarations from enclosing scopes, enabling
    cross-module method registration (e.g. a function in an outer module that
    takes an inner module's eponymous type). *)
let rec pre_register_methods_from_structure
    tbl
    cands
    ~at_top_level
    (parent_decls : (Label.t * Miniml.ml_structure_elem) list)
    (sel : (Label.t * Miniml.ml_structure_elem) list) : unit =
      ...
      | SEmodule m ->
        ...
        | MEfunctor (_mbid, _mt, body) ->
          (* Recurse into functor bodies — they may contain modules with
             eponymous types. *)
          pre_register_methods_from_module_expr
            tbl
            cands
            (parent_decls @ sel)
            body
...
(** Recurse into module expressions (struct bodies and functor bodies) to find
    nested modules with eponymous types. *)
and pre_register_methods_from_module_expr
    tbl
    cands
    (parent_decls : (Label.t * Miniml.ml_structure_elem) list) = function
  | MEstruct (_mp, sel) ->
    pre_register_methods_from_structure
      tbl
      cands
...
(** After the full structure has been scanned for methods, determine which
    ...
    In C++, when an inductive type has type parameters, those parameters become
    template parameters on the struct. If a method's return type involves a type
    variable that is NOT one of the inductive's parameters (i.e. it is an
    "indexed" type variable), it cannot be expressed in the C++ type system and
    must be erased to [std::any].

    For example, consider: [Inductive Foo (A : Type) := mk : A -> Foo A.]
    [Definition bar (A B : Type) (x : Foo A) : B := ...] Here [B] is not a
    parameter of [Foo], so [bar]'s return type is erased to [std::any] in the
    method signature.
...
let compute_returns_any tbl (s : ml_structure) =
  ...
  (* Step 2: For each registered method, find its ML type and check whether the
     return type is erased. *)
  Hashtbl.iter
    (fun func_ref info ->
...
(** Build the complete method registry from the full extraction structure.
    ...
    [do_struct_with_decl_tracking] before any C++ code is rendered.
