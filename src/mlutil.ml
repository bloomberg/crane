...
| (Tarr _ | Tglob _ | Tvar _ | Tvar' _ | Tmeta _ | Tdummy _ | Tunknown | Taxiom | Tstring), _
  -> false
...
let dump_unused_vars a =
  let rec ren env a = match a with
    ...
    | MLlam (id,t,b) ->
       ...
       if !occ_id then if b' == b then a else MLlam(id,t,b')
       else MLlam(Dummy,t,b')
    ...
    and ren_branch env ((ids,r,p,b) as tr) =
      ...
      let ids' =
        List.map2
          (fun (id, ty) occ -> if !occ then (id, ty) else (Dummy, Tdummy Ktype)) (*TODO: I am making up the kill reason *)
          ids occs
      in
      if b' == b && List.equal (fun (x1, _) (x2, _) -> eq_ml_ident x1 x2) ids ids' then tr
      else (ids',r,p,b')
  in
  ren [] a
...
let has_deep_pattern br =
  ...
  in
  Array.exists (function (_,_,pat,_) -> deep pat) br
...
let rec many_lams id a = function
  | 0 -> a
  | n -> many_lams id (MLlam (id, Taxiom, a)) (pred n) (* TODO: Taxiom probs bad *)
...
let census_add, census_max, census_clean =
  ...
  | [] -> raise Not_found
  | (k', s) as p :: l ->
    if eq_ml_ast (fst k) (fst k') then (k', Int.Set.add v s) :: l
    else p :: add k v l
  in
  let addf k i =
    try h := add k i !h
  ...
  in
  let maxf () =
    let len = ref 0 and lst = ref Int.Set.empty and elm = ref (MLaxiom "should not appear", Taxiom) in
    List.iter
      (fun (e, s) ->
...
let iota_gen br hd =
  let rec iota k = function
    ...
    | MLcase(typ,e,br') ->
        let new_br =
          Array.map (fun (i,r,p,c)->(i,r,p,iota (k+(List.length i)) c)) br'
...
(*
let kill_some_lams' bl tys (ids,c) =
  let n = List.length bl in
  let n' = List.fold_left (fun n b -> if b == Keep then (n+1) else n) 0 bl in
  if Int.equal n n' then ids,tys,c
  else if Int.equal n' 0 && not (List.exists is_impl_kill bl)
  then [],ast_lift (-n) c
  else begin
    let v = Array.make n None in
    let rec parse_ids i j = function
      ...
      | Kill _ :: l -> parse_ids (i+1) j l
    in parse_ids 0 1 bl;
    select_via_bl bl (List.combine ids tys), gen_subst v (n'-n) c
  end
*)
...
let eta_expansion_sign s (ids,c) =
  let rec abs ids rels i = function
    ...
    | Keep :: l -> abs ((anonymous, Taxiom) :: ids) (MLrel i :: rels) (i+1) l (* TODO: Taxiom is probably bad here!!! *)
...
let ml_size_branch size pv = Array.fold_left (fun a (_,_,_,t) -> a + size t) 0 pv
