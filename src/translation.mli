...
(** Binder-type state: every binder's C++ type, tagged with what decided it. *)
type binder_env =
  (cpp_type * Translation_state.binder_origin) Translation_state.IntMap.t
...
(** A type variable that really denotes an associated type of a type-class
    instance parameter: [htp_tvar] stands for
    [typename <htp_instance>::<htp_field>]. *)
...
(** Install the lookup ([Cpp_names.lookup_method_ind_tvar_positions]) giving
    the type-variable positions a methodified reference's receiver already
    fixes.  Called by {!Cpp_print} at load time, as above. *)
val set_method_ind_tvars_lookup : (GlobRef.t -> int list) -> unit
