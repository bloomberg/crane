...
__attribute__((pure)) unsigned int MutualRecursion::tree_sum(
  ...
  if (std::holds_alternative<
    ...
    const auto &_m =
        *std::get_if<typename MutualRecursion::tree<unsigned int>::Leaf>(
            &t->v());
    return _m.d_a0;
  } else {
    const auto &_m =
        *std::get_if<typename MutualRecursion::tree<unsigned int>::Node>(
            &t->v());
    return forest_sum(_m.d_a0);
  }
}
...
__attribute__((pure)) unsigned int MutualRecursion::forest_sum(
  ...
  if (std::holds_alternative<
    ...
    const auto &_m =
        *std::get_if<typename MutualRecursion::forest<unsigned int>::Trees>(
            &f->v());
