...
int main() {
  ...
  // Test 3: Sort two elements
  {
    auto input = vector_to_list({2, 1});
    auto result = psort(input);
    auto sorted_list = result->_a0;
    auto vec = list_to_vector(sorted_list);
    ASSERT(vec.size() == 2);
    ASSERT(vec[0] == 1 && vec[1] == 2);
    std::cout << "Test 3 (two elements): PASSED" << std::endl;
  }

  // Test 4: Sort already sorted list
  {
    ...
    std::cout << "Test 4 (already sorted): PASSED" << std::endl;
  }

  // Test 5: Sort reverse sorted list
  {
    ...
    std::cout << "Test 5 (reverse sorted): PASSED" << std::endl;
  }

  // Test 6: Sort unsorted list
  {
    ...
    std::cout << "Test 6 (unsorted list): PASSED" << std::endl;
  }

  if (testStatus == 0) {
    std::cout << "\nAll pair sort tests passed!" << std::endl;
  } else {
