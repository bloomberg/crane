...
(* Definition test : unit -> string :=
  fun _ => ToString.list_to_string (ToString.list_to_string string_of_nat)
    (TopologicalSort.topological_sort_graph Nat.eqb
      [ (one, [two; three])
      ; (two, [four])
      ; (three, [four; five])
      ; (four, [six])
...
(*
Eval compute in (TopologicalSort.topological_sort_graph Nat.eqb [(1, [2]); (2, [])]).
Eval compute in (TopologicalSort.topological_sort_graph Nat.eqb [(1, [2]); (2, [1])]).

Eval compute in (TopologicalSort.topological_sort_graph Nat.eqb
                   [ (1, [2; 3]); (2, [1]); (3, []) ]).
...
Crane Extraction "topological_sort" TopologicalSort ToString.
