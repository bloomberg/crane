  ...
  template <typename T1> std::shared_ptr<List<T1>> concat() const {
    if (std::holds_alternative<typename List<std::shared_ptr<List<T1>>>::Nil>(
      ...
      const auto &[d_a0, d_a1] =
          std::get<typename List<std::shared_ptr<List<T1>>>::Cons>(this->v());
    ...
    get_elems_aux =
      ...
      if (std::holds_alternative<typename List<std::pair<T1, T1>>::Nil>(
        ...
        if (f1.has_value()) {
          ...
          if (f2.has_value()) {
            const T1 &_x0 = *f2;
            return get_elems_aux(d_a1, std::move(h));
          } else {
            return get_elems_aux(d_a1, List<T1>::cons(e2, h));
          }
        } else {
          if (f2.has_value()) {
            const T1 &_x = *f2;
            return get_elems_aux(d_a1, List<T1>::cons(e1, h));
          } else {
            if (eqb_node(e1, e2)) {
              return get_elems_aux(d_a1, List<T1>::cons(e1, h));
            } else {
              return get_elems_aux(d_a1,
  ...
  __attribute__((pure)) static std::optional<T1> cycle_entry(
    ...
    if (std::holds_alternative<
      ...
      const auto &[d_a0, d_a1] = std::get<
          ...
          graph0->v());
      const T1 &e = d_a0.first;
      const std::shared_ptr<List<T1>> &_x0 = d_a0.second;
