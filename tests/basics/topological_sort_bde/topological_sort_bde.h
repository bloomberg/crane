...
List clone() const {
  ...
  while (!_stack.empty()) {
    ...
    if (std::holds_alternative<Nil>(_src->v())) {
      ...
      _dst->d_v_ = Cons{_alt.d_a0, _alt.d_a1 ? std::make_unique<List>() : nullptr};
      auto &_dst_alt = std::get<Cons>(_dst->d_v_);
      if (_alt.d_a1) _stack.push_back({_alt.d_a1.get(), _dst_alt.d_a1.get()});
    }
  }
  return _out;
}
...
T1>
static bool null(const List<T1>& xs){if (bsl::holds_alternative<typename List<T1>::Nil>(xs.v())) {
return true;
} else {
return false;
}}template <typename T1, MapsTo<bool, T1, T1>
