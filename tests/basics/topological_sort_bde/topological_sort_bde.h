...
explicit List(const List<_U>& _other) {
  if (std::holds_alternative<typename List<_U>::Nil>(_other.v())) {
    ...
    d_v_ = Cons{t_A(d_a0), d_a1 ? std::make_unique<List<t_A>>(*d_a1) : nullptr};
  }
}
...
bsl::make_unique<List<t_A>>(bsl::move(a1))});}
  // MANIPULATORS
~List() {
  ...
  auto _drain = [&](List &_node) {
      if (std::holds_alternative<Cons>(_node.d_v_)) {
        ...
        if (_alt.d_a1) _stack.push_back(std::move(_alt.d_a1));
      }
  };
  _drain(*this);
  while (!_stack.empty()) {
...
bsl::move(h)); } else { return get_elems_aux(d_a1_value, List<T1>::cons(e2,
bsl::move(h))); } } else { if (f2.has_value()) { T1 _x = *f2; return get_elems_aux(d_a1_value,
List<T1>::cons(e1, bsl::move(h))); } else { if (eqb_node(e1,
e2)) { return get_elems_aux(d_a1_value, List<T1>::cons(e1,
bsl::move(h))); } else { return get_elems_aux(d_a1_value, List<T1>::cons(e1,
...
return cycle_entry_aux<T1>(eqb_node, graph0, List<T1>::cons(elem,
bsl::move(seens)), d_a0, c);
} } }}template <typename T1, MapsTo<bool, T1, T1>
F0>
...
T1 e = std::pair<T1, List<T1>>(d_a0).first; List<T1> _x0 = std::pair<T1, List<T1>>(d_a0).second; return bsl::make_optional<T1>(cycle_entry_aux<T1>(eqb_node,
...
return List<List<T1>>::cons(bsl::move(mins_),
