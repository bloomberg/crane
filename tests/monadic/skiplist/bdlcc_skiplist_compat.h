...
// - "R" (reverse search) methods are not implemented; they delegate to forward
// search
...
template <class KEY, class DATA> class SkipList;
...
template <class KEY, class DATA> class SkipListPair {
private:
...
public:
  ...
  std::shared_ptr<SkipNode<KEY, DATA>> node() const { return d_node; }
};
...
private:
  ...
  static std::shared_ptr<Pair>
  wrapNode(std::shared_ptr<SkipNode<KEY, DATA>> node) {
    if (!node)
      return nullptr;
  ...
  template <typename F> static auto atomic(F &&f) {
...
public:
  ...
  int addUnique(const KEY &key, const DATA &data,
                bool *newFrontFlag = nullptr) {
    return atomic([&]() -> int {
      unsigned int level = randomLevel();
  ...
  // Reverse search variants (delegate to forward search - "R" optimization not
  // implemented)
  ...
  int addUniqueR(const KEY &key, const DATA &data,
                 bool *newFrontFlag = nullptr) {
  ...
  int removeAll() {
    return atomic(
        [&]() -> int { return static_cast<int>(d_impl->bde_removeAll()); });
  }
  ...
  int back(PairHandle *back) const {
    return atomic([&]() -> int {
      ...
      if (result.first == e_SUCCESS && back && result.second.has_value()) {
        *back =
            PairHandle(const_cast<SkipList *>(this), wrapNode(*result.second));
      }
      return result.first;
    });
  }
  ...
  bool exists(const KEY &key) const {
    return atomic(
        [&]() -> bool { return d_impl->bde_exists(keyLt, keyEq, key); });
  }
  ...
  bool isEmpty() const {
    return atomic([&]() -> bool { return d_impl->bde_isEmpty(); });
  }

  /// Return the number of items in this list.
  int length() const {
    return atomic(
        [&]() -> int { return static_cast<int>(d_impl->bde_length()); });
  }
  ...
  int findR(PairHandle *item, const KEY &key) const { return find(item, key); }
  ...
  int next(PairHandle *next, const Pair *reference) const {
    ...
    return atomic([&]() -> int {
      auto result =
          ::SkipList<int, int>::template bde_next<KEY, DATA>(reference->node());
