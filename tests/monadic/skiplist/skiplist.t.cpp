...
bool test_concurrent_producer_consumer() {
  ...
  if (consumed != NUM_ITEMS) {
    std::cerr << "Expected " << NUM_ITEMS << " consumed, got " << consumed
              << std::endl;
    return false;
  }
...
bool test_concurrent_mixed_operations() {
  ...
  // Basic sanity checks
  if (len > 0 && isEmpty) {
    std::cerr << "Inconsistent state: length=" << len << " but isEmpty=true"
              << std::endl;
    return false;
  }
  if (len == 0 && !isEmpty) {
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "test_concurrent_insert: " << (r14 ? "PASS" : "FAIL")
            << std::endl;
  if (r14)
    passed++;

  bool r15 = test_concurrent_read_write();
  std::cout << "test_concurrent_read_write: " << (r15 ? "PASS" : "FAIL")
            << std::endl;
  if (r15)
    passed++;

  bool r16 = test_concurrent_producer_consumer();
  std::cout << "test_concurrent_producer_consumer: " << (r16 ? "PASS" : "FAIL")
            << std::endl;
  if (r16)
    passed++;

  bool r17 = test_concurrent_mixed_operations();
  std::cout << "test_concurrent_mixed_operations: " << (r17 ? "PASS" : "FAIL")
            << std::endl;
  if (r17)
    passed++;

  std::cout << "\nSkipList tests passed: " << passed << "/" << total
            << std::endl;
  ...
  if (run_bench) {
    std::cout << "\n=== Performance Benchmarks (Raw STM SkipList) ==="
              << std::endl;

    auto timer = []() { return std::chrono::high_resolution_clock::now(); };
    ...
    // Benchmark 1: Sequential insert
    {
      ...
      std::cout << std::left << std::setw(30)
                << "Sequential insert (10000):" << std::right << std::setw(10)
                << std::fixed << std::setprecision(2) << elapsed_ms(start)
                << " ms" << std::endl;
    }
    ...
    // Benchmark 2: Random insert
    {
      ...
      std::cout << std::left << std::setw(30)
                << "Random insert (10000):" << std::right << std::setw(10)
                << std::fixed << std::setprecision(2) << elapsed_ms(start)
                << " ms" << std::endl;
    }
    ...
    // Benchmark 6: Concurrent insert (4 threads)
    {
      ...
      for (auto &th : threads)
        th.join();
      std::cout << std::left << std::setw(30)
                << "Concurrent insert (4T, 1000):" << std::right
    ...
    // Benchmark 7: Concurrent lookup (4 threads)
    {
      ...
      for (auto &th : threads)
        th.join();
      std::cout << std::left << std::setw(30)
                << "Concurrent lookup (4T, 10000):" << std::right
                << std::setw(10) << std::fixed << std::setprecision(2)
                << elapsed_ms(start) << " ms" << std::endl;
    }
    ...
    // Benchmark 8: Mixed concurrent (2 writers, 2 readers)
    {
      ...
      for (auto &th : threads)
        th.join();
      std::cout << std::left << std::setw(30)
                << "Mixed concurrent (4T):" << std::right << std::setw(10)
                << std::fixed << std::setprecision(2) << elapsed_ms(start)
                << " ms" << std::endl;
    }
    ...
    // Benchmark 9: Producer-consumer
    {
      ...
      consumer.join();
      std::cout << std::left << std::setw(30)
                << "Producer-consumer (1000):" << std::right << std::setw(10)
                << std::fixed << std::setprecision(2) << elapsed_ms(start)
                << " ms" << std::endl;
    }
