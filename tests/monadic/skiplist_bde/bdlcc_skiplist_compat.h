...
// - Uses bsl::shared_ptr for reference counting instead of manual ref counting
...
class SkipListPair {
  ...
  public:
    ...
    bsl::shared_ptr<SkipNode<KEY, DATA>> node() const {
        return d_node;
    }
};
...
class SkipList {
  ...
  private:
    ...
    static bsl::shared_ptr<Pair> wrapNode(bsl::shared_ptr<SkipNode<KEY, DATA>> node) {
