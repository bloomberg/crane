...
// Crane-extracted SkipList implementation. It translates between the STM-based
// functional interface and the BDE output-parameter style.
//
// Key differences from the actual BDE implementation: - Uses STM for thread
// safety instead of mutex locks - Uses bsl::shared_ptr for reference counting
// instead of manual ref counting - "R" (reverse search) methods are not
// implemented; they delegate to forward search - KEY type must have both
// operator< and operator== defined
//
...
/// Opaque type representing a reference to an element in the SkipList.  This
/// wraps a shared_ptr to a SkipNode, providing key() and data() accessors.
template <class KEY, class DATA>
class SkipListPair {
  private:
    bsl::shared_ptr<SkipNode<KEY, DATA> > d_node;

    friend class SkipList<KEY, DATA>;
    friend class SkipListPairHandle<KEY, DATA>;

    explicit SkipListPair(bsl::shared_ptr<SkipNode<KEY, DATA> > node)
    : d_node(std::move(node))
    {
    }

  public:
    /// Return a copy of the "data" of this pair (STM-protected value).
    DATA data() const { return stm::readTVar<DATA>(d_node->value); }

    /// Return a reference to the non-modifiable "key" value of this pair.
    const KEY& key() const { return d_node->key; }

    /// Return the underlying node pointer (for internal use).
    bsl::shared_ptr<SkipNode<KEY, DATA> > node() const { return d_node; }
};
...
/// RAII handle for SkipList pair references. Provides automatic cleanup and
...
class SkipListPairHandle {
  ...
  private:
    ...
    bsl::shared_ptr<Pair>  d_pair;

    friend class SkipList<KEY, DATA>;

    SkipListPairHandle(SkipList<KEY, DATA> *list, bsl::shared_ptr<Pair> pair)
    : d_list_p(list)
    , d_pair(std::move(pair))
    {
    }
  ...
  public:
    ...
    operator const Pair *() const { return d_pair.get(); }

    /// Return a copy of the "data" value of the pair (STM-protected).
    DATA data() const { return d_pair->data(); }

    /// Return a reference to the non-modifiable "key" value of the pair.
    const KEY& key() const { return d_pair->key(); }

    /// Return true if this handle refers to a valid pair.
    bool isValid() const { return d_pair != nullptr; }
};
...
/// BDE-compatible wrapper around Crane's SkipList implementation.  Provides
/// the same interface as bdlcc::SkipList with output parameters and integer
/// return codes.
template <class KEY, class DATA>
class SkipList {
  public:
    // CONSTANTS
    enum { e_SUCCESS = 0, e_NOT_FOUND = 1, e_DUPLICATE = 2, e_INVALID = 3 };
  ...
  private:
    ...
    static bsl::shared_ptr<Pair> wrapNode(
                                    bsl::shared_ptr<SkipNode<KEY, DATA> > node)
    {
        if (!node)
            return nullptr;
        return bsl::shared_ptr<Pair>(new Pair(std::move(node)));
    }
  ...
  public:
    ...
    /// Add the key/data pair only if key is not already present.  Return 0 on
    /// success, e_DUPLICATE if key exists.
    ...
    /// Add the key/data pair only if key is not already present.  Load result
    /// into PairHandle. Return 0 on success.
    ...
    // Reverse search variants (delegate to forward search - "R" optimization
    // not implemented)
    ...
    /// Remove the first item from the list.  Return 0 on success, non-zero if
    /// list is empty.
    ...
    /// Remove the item identified by the pair.  Return 0 on success,
    /// e_NOT_FOUND if not in list.
    ...
    /// Remove all items from this list.  Return the number of items removed.
    int removeAll()
    {
        return atomic([&]() -> int {
    ...
    /// Load a reference to the last item into back.  Return 0 on success,
    /// non-zero if list is empty.
    ...
    /// Load a reference to the first item into front.  Return 0 on success,
    /// non-zero if list is empty.
    ...
    /// Find element with the specified key.  Return 0 on success, non-zero if
    /// not found.
    ...
    /// Load into next a reference to the item after reference.  Return 0 on
    /// success, non-zero if reference is at back.
    int next(PairHandle *next, const Pair *reference) const
    {
        if (!reference)
            return e_NOT_FOUND;
        return atomic([&]() -> int {
            auto result = ::SkipList<int, int>::template bde_next<KEY, DATA>(
                                                            reference->node());
            if (result.first == e_SUCCESS && next &&
                result.second.has_value()) {
                *next = PairHandle(const_cast<SkipList *>(this),
                                   wrapNode(*result.second));
            }
            return result.first;
        });
    ...
    /// Load into prevPair a reference to the item before reference.  Return 0
    /// on success, non-zero if reference is at front.
    ...
    /// If item is not at front, load the previous item into item; otherwise
    /// reset. Return 0 on success, e_NOT_FOUND if item is no longer in list.
  ...
  private:
    ...
    {
        // Use thread-local RNG for thread safety Seed with a hash of thread ID
        // for deterministic but varied seeds
