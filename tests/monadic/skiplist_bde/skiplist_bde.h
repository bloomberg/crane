...
(*(this)).SkipList::slHead, target, lvl, path);}
...
newN);
if (curLvl < newLevel) { stm::writeTVar((*(this)).SkipList::slLevel, newLevel);
...
V>(eqK, 10000u, first, (*(this)).SkipList::slHead,
...
newN);
[&]() -> void {
if (curLvl < level) { stm::writeTVar((*(this)).SkipList::slLevel, level);
return; } else { return; }
}();
