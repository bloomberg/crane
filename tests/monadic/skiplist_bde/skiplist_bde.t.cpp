...
// Adapted from BDE skiplist tests for Crane's SkipList implementation
// Tests for unimplemented methods (Raw, update) are commented out
...
#include "bdlcc_skiplist_compat.h"
// Note: Using bdlcc_compat namespace instead of bdlcc
...
void aSsErT(int c, const char *s, int i) {
  if (c) {
    cout << "Error " << __FILE__ << "(" << i << "): " << s << "    (failed)"
         << endl;
...
// accomodate because `bdlcc_compat::SkipList` lacks a `comparator` functor
...
#define U_TEST_TYPES_PRIMITIVE                                                 \
  signed char, size_t, bsltf::TemplateTestFacility::ObjectPtr,                 \
...
#define U_WINDOWS_TEST_TYPES_USER_DEFINED                                      \
  bsltf::EnumeratedTestType::Enum, bsltf::UnionTestType, bsltf::AllocTestType, \
      bsltf::BitwiseMoveableTestType, bsltf::AllocBitwiseMoveableTestType,     \
      bsltf::MovableAllocTestType, bsltf::NonTypicalOverloadsTestType

#if defined(BSLS_PLATFORM_OS_WINDOWS) || defined(BDE_BUILD_TARGET_TSAN)
#define U_TEST_TYPES_REGULAR                                                   \
  U_TEST_TYPES_PRIMITIVE, U_WINDOWS_TEST_TYPES_USER_DEFINED
#else
#define U_TEST_TYPES_REGULAR                                                   \
  U_TEST_TYPES_PRIMITIVE, BSLTF_TEMPLATETESTFACILITY_TEST_TYPES_USER_DEFINED
#endif
...
enum AddMode {
  ...
  // UNIMPLEMENTED: Unique raw methods without PairHandle
  // e_ADD_UNIQUE_RAW,                                // 0
  ...
  // UNIMPLEMENTED: removeAll with vector parameter
  // e_ADD_WITH_REMOVEALL_HANDLES = e_END,           // no such function

  e_END2
};

// UNIMPLEMENTED: Assertion for commented-out enum value
...
const struct Data {
  ...
  const char *d_spec_p;
  bool d_unique;
} DATA[] = {{L_, "", 1},      {L_, "7", 1},    {L_, "8", 1},
            {L_, "78a", 1},   {L_, "788a", 0}, {L_, "78a78a", 0},
            {L_, "78abcd", 1}};
...
public:
  ...
  static int getDeleteCount() { return s_deleteCount; }
...
template <class TYPE> class TestObject {
...
template <class TYPE> TestObject<TYPE>::~TestObject() {
  d_ob.address()->~TYPE();
}

// MANIPULATORS
template <class TYPE> TYPE *TestObject<TYPE>::address() {
  return d_ob.address();
}

template <class TYPE> TYPE &TestObject<TYPE>::object() { return d_ob.object(); }

// ACCESSOR
template <class TYPE> bslma::Allocator *TestObject<TYPE>::allocator() {
...
public:
  // CREATOR
  explicit Disp(int value) : d_value(value) {
    ASSERT(0 <= value);
    ASSERT(value < 128);
  }
...
public:
  ...
  explicit RandGen(int startSeed);
...
// CREATOR
inline RandGen::RandGen() : d_seed(++s_globalSeed), d_bits(0) {
  (void)(*this)();
  (void)(*this)();
  d_seed ^= d_seed >> 32;
  (void)(*this)();
...
inline RandGen::RandGen(int startSeed) : d_seed(startSeed) {
  (void)(*this)();
  (void)(*this)();
  d_seed ^= d_seed >> 32;
  (void)(*this)();
...
inline unsigned RandGen::bits(unsigned bits) {
...
template <class EnumType, unsigned MOD> struct RandGenMod {
  // DATA
...
template <class EnumType, unsigned MOD>
inline EnumType RandGenMod<EnumType, MOD>::operator()(RandGen *randGen_p) {
  unsigned ret;
  do {
...
VectorRandGen::VectorRandGen() : d_size(1 << 16), d_mask(d_size - 1), d_idx(0) {
  RandGen rg;
...
inline bsl::ostream &operator<<(bsl::ostream &stream, u::VecType value) {
...
template <class KEY_TYPE, class DATA_TYPE = KEY_TYPE> class TestDriver {
  // PRIVATE TYPES
  typedef bdlcc_compat::SkipList<KEY_TYPE, DATA_TYPE> Obj;
  ...
  enum {
    k_OBJ_IS_PRINTABLE = !bsl::is_same<KEY_TYPE, const char *>::value &&
  ...
  static int addByMode(Obj *dst, u::AddMode addMode, int keyValue,
                       int numPreExisting, u::RandGen *randGen, bool checkFront,
  ...
  static void checkContainer(const Obj &mX,
                             const int (&numNodesForKeyVal)[k_KEY_VALUE_MOD]);
  static void
  checkContainer(const Obj &mX,
  ...
  static Obj &hexGg(Obj *dst, const char *spec, int level);
  ...
  static bool hexVerify(const Obj &src, const char *spec);
  ...
  static void populate(Obj *dst, const TableRecord *array,
  ...
  static void dump(const Obj &list, const TableRecord *table = 0,
  ...
  /// in the process.
  static int raiseMaximumLevel(Obj *dst);
  ...
  static void verify(const Obj &list, const TableRecord *array,
                     const bsl::size_t length, const int line);
  ...
  static void verifyR(const Obj &list, const TableRecord *array,
                      const bsl::size_t length, const int line);
...
public:
  ...
  static void manyThreadsRandomModesOnOneSkipListThread(
      Obj *mX_p, bsls::AtomicInt *numNodesForKeyVal, bslmt::Barrier *barrier,
  ...
  // UNIMPLEMENTED: Raw pointer methods
  // static void skipTest();

  // UNIMPLEMENTED: updateR method not implemented
  // static void updateRTest();

  static void addRTest();

  template <class VECTOR> static void removeAllTestByVecType();
  ...
  // UNIMPLEMENTED: update and updateR methods not implemented
...
int TestDriver<KEY_TYPE, DATA_TYPE>::addByMode(Obj *dst, u::AddMode addMode,
                                               int keyValue, int numPreExisting,
                                               u::RandGen *randGen,
                                               bool checkFront,
                                               bslma::Allocator *alloc) {
  ...
  if (0 == rc) {
    ...
    // UNIMPLEMENTED: level checking requires access to node internals
    // if (0 <= level && (ph || pr)) {
    //     typedef bdlcc_compat::SkipList_Node<KEY_TYPE, DATA_TYPE> Node;
    //
    ...
    // UNIMPLEMENTED: Raw pointer handling
    // if (pr) {
    //     ASSERT(pr->key()  == key);
    //     ASSERT(pr->data() == data);
    //
...
void TestDriver<KEY_TYPE, DATA_TYPE>::checkContainer(
    const Obj &mX, const int (&numNodesForKeyVal)[k_KEY_VALUE_MOD]) {
  PairHandle ph;
...
void TestDriver<KEY_TYPE, DATA_TYPE>::checkContainer(
  ...
  int *ret =
      bsl::copy(numNodesForKeyVal + 0, numNodesForKeyVal + k_KEY_VALUE_MOD,
...
// UNIMPLEMENTED: hexGg uses addRaw, addAtLevelRaw, releaseReferenceRaw
/*
template <class KEY_TYPE, class DATA_TYPE>
bdlcc_compat::SkipList<KEY_TYPE, DATA_TYPE>&
...
// UNIMPLEMENTED: populate method uses dst->allocator() accessor
/*
template <class KEY_TYPE, class DATA_TYPE>
...
// UNIMPLEMENTED: raiseMaximumLevel uses addAtLevelRaw and SkipList_Node
template <class KEY_TYPE, class DATA_TYPE>
int TestDriver<KEY_TYPE, DATA_TYPE>::raiseMaximumLevel(Obj *dst) {
  // No-op implementation since addAtLevelRaw is not implemented
  (void)dst;
  return 0;
}

// UNIMPLEMENTED: verify method uses list.allocator() accessor
/*
template <class KEY_TYPE, class DATA_TYPE>
                                             ...
                                             const int          line)
{
    ...
    // UNIMPLEMENTED: frontRaw not supported
    // list.frontRaw(&pr);
    list.front(&skPh);
    list.front(&nePh);
    ...
    for (int ii = 0; ii < iterations; ++ii) {
        ...
        // UNIMPLEMENTED: skipForwardRaw not supported
...
// UNIMPLEMENTED: verifyR method uses list.allocator() accessor
/*
template <class KEY_TYPE, class DATA_TYPE>
                                              ...
                                              const int          line)
{
    ...
    // UNIMPLEMENTED: backRaw not supported
    // list.backRaw(&pr);
    list.back(&skPh);
    list.back(&pvPh);
    ...
    for (int ii = length - 1; end <= ii; --ii) {
        ...
        // UNIMPLEMENTED: skipBackwardRaw not supported
...
void TestDriver<KEY_TYPE, DATA_TYPE>::testIndependentAddFunctions() {
  if (verbose)
    cout << "testIndependentAddFunctions<" << bsls::NameOf<KEY_TYPE>() << ", "
  ...
  for (int ai = u::e_BEGIN; ai < u::e_END; ++ai) {
    ...
    static int keys[] = {8,  10, 20, 15, 14, 16, 4,  25, 20, 17, 25,
                         8,  20, 18, 23, 25, 21, 3,  11, 33, 20, 31,
                         4,  33, 16, 19, 17, 12, 8,  9,  10, 40, 16,
                         25, 26, 2,  40, 8,  9,  11, 10, 12};
    ...
    for (int ti = 0; ti < k_NUM_KEYS; ++ti) {
      ...
      int rc =
          TD::addByMode(&mX, addMode, keyVal, numNodes, &randGen, true, &va);
...
void TestDriver<KEY_TYPE, DATA_TYPE>::testMultiAddFunctions() {
  if (verbose)
    cout << "testMultiAddFunctions<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
...
// UNIMPLEMENTED: manyThreadsRandomModesOnOneSkipListMain uses constructor with
// allocator parameter
/*
template <class KEY_TYPE, class DATA_TYPE>
...
template <class KEY_TYPE, class DATA_TYPE>
void TestDriver<KEY_TYPE, DATA_TYPE>::manyThreadsRandomModesOnOneSkipListThread(
    Obj *mX_p, bsls::AtomicInt *numNodesForKeyVal, bslmt::Barrier *barrier,
    bsls::AtomicBool *doneFlag) {
  int threadId = u::masterThreadId++;

  if (veryVeryVerbose)
    COUT << "manyThreadsRandomModesOnOneSkipListThread<"
         << bsls::NameOf<KEY_TYPE>() << ", " << bsls::NameOf<DATA_TYPE>()
         << ">(): id " << threadId << ";\n";
  ...
  while (!*doneFlag) {
    ...
    int rc = TD::addByMode(mX_p, addMode, keyVal, -1, &randGen, false, &va);
...
// UNIMPLEMENTED: valueSemanticTest uses hexGg which uses addRaw, addAtLevelRaw,
// releaseReferenceRaw
/*
template <class KEY_TYPE, class DATA_TYPE>
...
// UNIMPLEMENTED: Raw pointer methods (skipForwardRaw, skipBackwardRaw, findRaw,
// etc.)
/*
template <class KEY_TYPE, class DATA_TYPE>
...
// ----------------------------------------------------------------------------
{
    ...
    for (int ii = 1; ii <= 6; ++ii) {
        ...
        if (exp) {
            // UNIMPLEMENTED: releaseReferenceRaw
...
// ---------------------------------------------------------------------------
{
    ...
    verify(X, check, U_LENGTH(check), L_);
    verifyR(X, checkB, U_LENGTH(check), L_);
...
{
  if (verbose)
    cout << "addRTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
  ...
  TableRecord init[] = {
      {L_, 5, 6, 0}, {L_, 3, 4, 0}, {L_, 6, 7, 0}, {L_, 4, 5, 0}, {L_, 8, 9, 0},
  };
...
void TestDriver<KEY_TYPE, DATA_TYPE>::removeAllTestByVecType() {
  static const u::VecType vecType =
      bsl::is_same<VECTOR, bsl::vector<PairHandle>>::value   ? u::e_BSL
      : bsl::is_same<VECTOR, std::vector<PairHandle>>::value ? u::e_STD
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
  ...
  if (verbose)
    cout << "  removeAllTestByVecType<" << vecType << "::vector<PairHandle>, "
         << bsls::NameOf<KEY_TYPE>() << ", " << bsls::NameOf<DATA_TYPE>()
         << ">();" << endl;

  TableRecord VALUES1[] = {
      {L_, 1, 2, 0}, {L_, 3, 4, 0}, {L_, 0, 1, 0}, {L_, 2, 3, 0}, {L_, 4, 5, 0},
  };

  TableRecord VALUES2[] = {
      {L_, 0, 1, 0}, {L_, 1, 2, 0}, {L_, 2, 3, 0}, {L_, 3, 4, 0}, {L_, 4, 5, 0},
  };
...
{
  if (verbose)
    cout << "removeAllTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
...
// ---------------------------------------------------------------------------
{
  ...
  if (verbose)
    cout << "findTest<" << keyType << ", " << dataType << ">();" << endl;
  ...
  {
    TableRecord init[] = {
        {L_, 1, 2, 0}, {L_, 5, 6, 0}, {L_, 3, 4, 0},
        {L_, 2, 3, 0}, {L_, 4, 5, 0},
    };

    TableRecord initCheck[] = {
        {L_, 1, 2, 0}, {L_, 2, 3, 0}, {L_, 3, 4, 0},
        {L_, 4, 5, 0}, {L_, 5, 6, 0},
    };
  ...
  // UNIMPLEMENTED: addRaw with null pointer not supported
  /*
  {
      u::RandGen randGen;
...
// ---------------------------------------------------------------------------
{
  ...
  if (verbose)
    cout << "findRTest<" << keyType << ", " << dataType << ">();" << endl;
  ...
  {
    static TableRecord init[] = {{L_, 2, 3, 0}, {L_, 5, 6, 0}, {L_, 4, 5, 0},
                                 {L_, 1, 2, 0}, {L_, 3, 4, 0}, {L_, 0, 1, 0}};
    bsl::vector<TableRecord> initCheck(init + 0, init + U_LENGTH(init), &ta);
    ...
    // UNIMPLEMENTED: update method not implemented
    /*
...
{
  if (verbose)
    cout << "iterationTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
...
// UNIMPLEMENTED: newFrontTest uses update, frontRaw, releaseReferenceRaw,
// nextRaw, ->key()
/*
template <class KEY_TYPE, class DATA_TYPE>
...
{
  if (verbose)
    cout << "ALLOCATION TEST\n"
            "===============\n";

  if (verbose)
    cout << "KEY_TYPE: " << bsls::NameOf<KEY_TYPE>()
...
// UNIMPLEMENTED: RemoveAllSafetyTest uses bsltf::AllocTestType which is not
// nothrow move constructible
/* namespace RemoveAllSafetyTest {

typedef bdlcc_compat::SkipList<int, bsltf::AllocTestType> Obj;
                         ...
                         u::AddMode  mode)
{
    ...
    for (int ii = 0; ii < numIterations; ++ii) {
        ...
        switch (mode) {
          ...
          case u::e_ADD_AT_LEVEL_RAW_PAIR: {
            list->add(&ph, key, data);  // No level parameter in compat wrapper
            ASSERT(ph);
          } break;
          case u::e_ADD_AT_LEVEL_RAW_R_PAIR: {
            list->addR(&ph, key, data);  // No level parameter in compat wrapper
            ASSERT(ph);
          } break;
          case u::e_ADD_AT_LEVEL_UNIQUE_RAW_PAIR: {
            int rc = list->addUnique(&ph, key, data);  // No level parameter in
compat wrapper ASSERT(!!ph == !rc); collisions += !!rc; } break; case
u::e_ADD_AT_LEVEL_UNIQUE_RAW_R_PAIR: { int rc = list->addUniqueR(&ph, key,
data);  // No level parameter in compat wrapper ASSERT(!!ph == !rc); collisions
+= !!rc; } break; case u::e_ADD_UNIQUE: { int rc = list->addUnique(key, data);
            collisions += !!rc;
          } break;
          case u::e_ADD_UNIQUE_R: {
        ...
        // UNIMPLEMENTED: Raw pointer handling and vector-based removeAll
removed
        // Just use the simple removeAll() without parameters
...
typedef bdlcc_compat::SkipList<int, u::CountedDelete> Obj;

bsls::AtomicInt masterId(0);

// UNIMPLEMENTED: frontRaw, addPairReferenceRaw, releaseReferenceRaw not
// supported
/*
...
///- - - - - - - - - - - - - - - -
// The "R" methods of `bdlcc_compat::SkipList` make it ideal for use in a
// scheduler, in which events are likely to be scheduled after existing events.
// In such an implementation, events are stored in the list with their scheduled
...
// also makes `bdlcc_compat::SkipList` well-suited to use in a scheduler - a
...
// simple scheduler class using `bdlcc_compat::SkipList`.  Note that the mutex
// in the scheduler is used only in connection with the scheduler's condition
// variable
// - thread-safe access to the `bdlcc_compat::SkipList` object does *not*
// require any synchronization.
//
// ```
class SimpleScheduler {
  // TYPES
  typedef bdlcc_compat::SkipList<bdlt::Datetime, bsl::function<void()>> List;
...
public:
  ...
  explicit SimpleScheduler(bslma::Allocator * /* basicAllocator */ = 0)
    ...
    int rc = bslmt::ThreadUtil::create(
        &d_dispatcher,
        bdlf::BindUtil::bind(&SimpleScheduler::dispatcherThread, this));
    BSLS_ASSERT(0 == rc);
    (void)rc;
    d_startBarrier.wait();
  }
...
void release(bdlcc_compat::SkipListPairHandle<int, int> *handle) {
  handle->release();
}

void test() {
  bdlcc_compat::SkipList<int, int> mX;
  bdlcc_compat::SkipList<int, int>::PairHandle handle;
  ...
  bslmt::ThreadUtil::create(&thread, bdlf::BindUtil::bind(&release, &handle));
...
public:
  ...
  explicit KeyValue(int i) : d_i(i) {}

  // ACCESSOR

  /// Return `true` if this object is less than the specified `rhs` and
  /// `false` otherwise.
  bool operator<(const KeyValue rhs) const { return d_i < rhs.d_i; }
};
...
typedef bdlcc_compat::SkipList<int, Payload> Obj;
...
// UNIMPLEMENTED: addRaw, releaseReferenceRaw not supported
/*
/// While the specified `done` is `false`, produce nodes on the specified
/// `list`.
void case16Produce (bdlcc_compat::SkipList<int, int> *list, bsls::AtomicInt
*done)
{
    int count = 0;
    while (!(*done)) {
        ...
        bdlcc_compat::SkipList<int, int>::Pair *h;
...
// UNIMPLEMENTED: frontRaw, findRRaw, ->key(), releaseReferenceRaw not supported
/*
/// While the specified `done` is `false`, from nodes from the specified
/// `list`.
void case16Consume(bdlcc_compat::SkipList<int, int> *list, bsls::AtomicInt
*done)
{
    while (!(*done)) {
        bdlcc_compat::SkipList<int, int>::Pair *h1;
        ASSERT(0 == list->frontRaw(&h1));
        bdlcc_compat::SkipList<int, int>::Pair *h2;
...
// UNIMPLEMENTED: addAtLevelRaw not supported
/*
/// Add the specified `length` items from the specified `array` to the
...
// UNIMPLEMENTED: frontRaw, ->key(), skipForwardRaw, releaseReferenceRaw not
// supported
/*
...
template <class SKIPLIST, class ARRAY>
void verifyReverse(const SKIPLIST &list, const ARRAY &array, int length,
                   int line) {
...
#define POPULATE_LIST(LP, A)                                                   \
  {                                                                            \
    TC::populate(LP, A, sizeof(A) / sizeof(A[0]));                             \
  }
...
typedef bdlcc_compat::SkipList<int, int> List;

enum { k_NUM_THREADS = 12, k_NUM_ITERATIONS = 100 };

// UNIMPLEMENTED: findRaw, ->key(), releaseReferenceRaw not supported
/*
...
typedef bdlcc_compat::SkipList<int, int> Obj;

// UNIMPLEMENTED: findRaw, update, releaseReferenceRaw not supported
/*
...
// UNIMPLEMENTED: test() calls updateNodes which is not implemented
/*
...
enum { k_NUM_REMOVE_NODE_THREADS = 10, k_NUM_REMOVE_ALL_THREADS = 10 };

typedef bdlcc_compat::SkipList<int, int> Obj;

// UNIMPLEMENTED: findRaw, releaseReferenceRaw not supported
/*
...
// UNIMPLEMENTED: test() calls removeNodes which is not implemented
/*
...
struct ThrashFunctor {
  ...
  explicit ThrashFunctor(Obj *list);
...
ThrashFunctor::ThrashFunctor(Obj *list)
    : d_list(*list), d_rand(masterRandSeed++) {}

ThrashFunctor::ThrashFunctor(const ThrashFunctor &original)
    : d_list(original.d_list), d_rand(masterRandSeed++) {}
...
enum FindExactMode { e_FIND, e_FINDR, e_FIND_EXACT_MOD };
...
enum MinorModifyMode { e_REMOVE, e_UPDATE, e_UPDATER, e_MINOR_MODIFY_MOD };
...
class ThrashFunctor {
  ...
  int cmpKeyAndData(Obj *list, const Pair *h, int key = -1,
                    bool flatten = false);
...
public:
  ...
  explicit ThrashFunctor(Obj *list, bslmt::Barrier *barrier);
...
// PRIVATE CLASS METHODS
inline int ThrashFunctor::dataForKey(int key) {
...
inline int ThrashFunctor::cmpKeyAndData(const PairHandle ph, int key,
                                        bool flatten) {
  int readKey = ph.key(); // key() returns const ref, not output param
  ...
  int data = ph.data(); // data() returns ref, not output param

  ASSERT(data == dataForKey(readKey));

  return flatten ? (readKey & ~k_VALUES_UPDATE_TOGGLE) : readKey;
}
...
int ThrashFunctor::addRand() {
  ...
  for (ii = 0; ii < k_LIMIT; ++ii) {
    ...
    case u::e_ADD_AT_LEVEL_RAW_PAIR: {
      d_list.add(&ph, key, data); // No level parameter
      ASSERT(ph);
    } break;
    case u::e_ADD_AT_LEVEL_RAW_R_PAIR: {
      d_list.addR(&ph, key, data); // No level parameter
      ASSERT(ph);
    } break;
    case u::e_ADD_AT_LEVEL_UNIQUE_RAW_PAIR: {
      rc = d_list.addUnique(&ph, key, data); // No level parameter
    } break;
    case u::e_ADD_AT_LEVEL_UNIQUE_RAW_R_PAIR: {
      rc = d_list.addUniqueR(&ph, key, data); // No level parameter
    } break;
    case u::e_ADD_UNIQUE: {
  ...
  if (ph) {
    ...
    // UNIMPLEMENTED: Obj::level() method not available
    // if (0 <= level) {
    ...
    // UNIMPLEMENTED: Raw pointer h not used
...
void ThrashFunctor::changeMajor() {
  ...
  // UNIMPLEMENTED: removeAll with parameter
  // case e_REMOVEALL: {
  //   PairHandle ph;
  ...
  //   }
  // } break;
  // UNIMPLEMENTED: removeAllRaw, releaseReferenceRaw
  ...
  //   }
  // } break;
  // UNIMPLEMENTED: Copy constructor and comparison operators not supported
  // case e_COPY_COMPARE: {
...
void ThrashFunctor::changeMinor() {
  ...
  // UNIMPLEMENTED: popFrontRaw, releaseReferenceRaw
  ...
  // UNIMPLEMENTED: addRaw, releaseReferenceRaw
...
void ThrashFunctor::findChangeMinorHandle(int key) {
  ...
  for (int ii = 0; ii < k_LIMIT; ++ii) {
    ...
    if (0 != rc) {
      ...
      enum { k_NUM_MISSES_BEFORE_ADD = 16, k_NUM_MISSES_BEFORE_VAGUE = 8 };
...
// UNIMPLEMENTED: findRaw, findRRaw, findLowerBoundRaw, findLowerBoundRRaw,
// findUpperBoundRaw, findUpperBoundRRaw, frontRaw, backRaw, nextRaw,
// previousRaw, skipForwardRaw, skipBackwardRaw, update, updateR,
// releaseReferenceRaw void ThrashFunctor::findChangeMinorRaw(int key)
// {
//     enum { k_LIMIT = 10 };
//
//     Pair *h;
...
// CREATORS
ThrashFunctor::ThrashFunctor(Obj *list, bslmt::Barrier *barrier)
    : d_list(*list), d_barrier(*barrier), d_rand(masterRandSeed++) {}

ThrashFunctor::ThrashFunctor(const ThrashFunctor &original)
    : d_list(original.d_list), d_barrier(original.d_barrier),
...
void ThrashFunctor::operator()() {
  ...
  for (ti = 0; !doneFlag; ++ti) {
    ...
    if (operationRand < findChangeMinorThreshold) {
      // UNIMPLEMENTED: findChangeMinorRaw uses raw pointers
      // if (d_rand.bits(1)) {
...
typedef bdlcc_compat::SkipList<bsls::TimeInterval, int> TimeQ;
...
// UNIMPLEMENTED: addRaw, releaseReferenceRaw, frontRaw, ->key() not supported
/*
...
// UNIMPLEMENTED: run() calls threadFunc which is not implemented
/*
/// run
void run()
{
...
template <class EnumType, unsigned MOD> void test() {
  u::RandGen rand;
...
struct Node {
  ...
  explicit Node(Node *next) : d_next_p(next) {}
};
...
struct ReadFunctor {
  static void setKillroy(Data *data_p);

  static bool swapOut(Node *p);
...
#endif
    void
    ReadFunctor::setKillroy(Data *data_p) {
...
struct Node {
  ...
  explicit Node(Node *next) : d_next_p(next), d_counter(0) {}
};
...
struct Functor {
  static bool checkCounter(Node *);

  static void incCounter(Node *p);

  static bool swapOut(Node *p);

  static bool swapIn(Node *p);
...
int main(int argc, char *argv[]) {
  ...
  case 32: {
    ...
    // UNIMPLEMENTED: popFrontRaw and ->key() not supported
    /*
    if (leak) {
        typedef bdlcc_compat::SkipList<int, int> Obj;
        Obj mX;
  ...
  case 31: {
    ...
    if (veryVeryVerbose) {
      ...
      TC::masterRandSeed = static_cast<int>(ti.totalNanoseconds() ^
                                            (ti.totalNanoseconds() >> 32));
    }
    if (verbose)
      P(TC::masterRandSeed);

    // UNIMPLEMENTED: ThrashFunctor uses Raw methods (addRaw, addUniqueRaw,
    // etc.)
    /*
  ...
  case 29: {
    ...
    // UNIMPLEMENTED: TC::test() uses unimplemented methods
    // TC::test();
  } break;
  case 28: {
    // --------------------------------------------------------------------
    ...
    // UNIMPLEMENTED: Case 27 uses manyThreadsRandomModesOnOneSkipListMain which
    // uses constructor with allocator parameter
    /* case 27: {
      // --------------------------------------------------------------------
      // THOROUGH MULTI-THREADED ADD TEST
      //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() - manyThreadsRandomModesOnOneSkipListMain,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
    ...
    // UNIMPLEMENTED: Case 26 uses test functions that use constructor with
    // allocator parameter
    /* case 26: {
      // --------------------------------------------------------------------
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() - testIndependentAddFunctions,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() - testMultiAddFunctions,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
    ...
    // UNIMPLEMENTED: Case 25 uses default constructor
    // case 25: {
    //   // --------------------------------------------------------------------
    ...
    //
    //   bdlcc_compat::SkipList<Test::KeyValue, Test::KeyValue> sl;    (void)
    //   sl;
    // } break;
    // UNIMPLEMENTED: Case 24 uses constructor with allocator parameter
    // case 24: {
    //   // --------------------------------------------------------------------
    ...
    //   P(ta.numBytesMax());
    // } break;
    // UNIMPLEMENTED: Case 23 uses allocator() accessor and constructor with
    // allocator parameter case 23: {
    //   // --------------------------------------------------------------------
    ...
    //                        "============================\n";
    //
    //   typedef bdlcc_compat::SkipList<int, bsl::string> Obj;
    //
    //   {
    ...
    //   }
    // } break;
    // UNIMPLEMENTED: Case 22 uses constructor with allocator parameter
    /* case 22: {
      // --------------------------------------------------------------------
    ...
    // UNIMPLEMENTED: Case 21 uses constructor with allocator and print() method
    /* case 21: {
      // --------------------------------------------------------------------
      ...
      // UNIMPLEMENTED: addRaw, findUpperBoundRaw not supported
      {
          u::RandGen randGen;
    ...
    // UNIMPLEMENTED: Case 20 uses constructor with allocator and print() method
    /*       case 20: {
            //
       --------------------------------------------------------------------
       ...
       --------------------------------------------------------------------
            ...
            typedef bdlcc_compat::SkipList<int, bsl::string> Obj;
            {
                ...
                Obj::PairHandle h;
                //validate error when trying to find something greater than max
       key ASSERT(mX.findLowerBound(&h, 10)); ASSERT(mX.findUpperBound(&h, 10));
  ...
  // UNIMPLEMENTED: Case 19 uses list.level() method
  /* case 19: {
    // --------------------------------------------------------------------
    // DISTRIBUTION TEST
    // --------------------------------------------------------------------
  ...
  case 18: {
    ...
    // UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
    // UNIMPLEMENTED: valueSemanticTest() - valueSemanticTest, UNIMPLEMENTED:
    // valueSemanticTest() -                         U_TEST_TYPES_REGULAR);

    // UNIMPLEMENTED: valueSemanticTest() -         TestDriver<int,
    // TTF::MethodPtr>::valueSemanticTest(); UNIMPLEMENTED: valueSemanticTest()
    // -         TestDriver<int, bsltf::AllocTestType>::valueSemanticTest();
    // UNIMPLEMENTED: valueSemanticTest() -         TestDriver<int,
    // bsltf::MovableAllocTestType>::valueSemanticTest(); UNIMPLEMENTED:
    // valueSemanticTest() -         TestDriver<int,
    // bsltf::SimpleTestType>::valueSemanticTest();
  } break;
  case 17: {
    // --------------------------------------------------------------------
    // UNIMPLEMENTED: Testing `skipBackward*` and `skipForward*`
    //
    ...
    /*
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         skipTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
    ...
    */
    ...
    // UNIMPLEMENTED: Case 16 uses constructor with allocator parameter
    /* case 16: {
      // ----------------------------------------------------
      ...
      // UNIMPLEMENTED: referenceTestFunc uses unimplemented methods
    ...
    // UNIMPLEMENTED: Case 15 uses constructor with allocator parameter
    /*       case 15: {
            // ------------------------------------
       ...
       "======================================================\n";
            ...
            for (int mi = u::e_BEGIN; mi < u::e_END2; ++mi) {
                ...
                ASSERTV(mode,
                         TC::removed, TC::collisions, TC::removed +
       TC::collisions, numThreads * numIterations, TC::removed + TC::collisions
  ...
  // UNIMPLEMENTED: Case 14 uses CountedDelete which is not nothrow move
  // constructible
  /* case 14: {
    // ------------------------------------
    // `remove` test
  ...
  // UNIMPLEMENTED: Case 13 uses constructor with allocator parameter
  /* case 13: {
    // ------------------------------------------
  ...
  case 12: {
    ...
    /*
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         updateRTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
  ...
  // UNIMPLEMENTED: Case 11 uses test functions with allocator constructor
  /* case 11: {
    // --------------------------------------------------------------------
    // ADDR TEST
    //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         addRTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
  ...
  // UNIMPLEMENTED: Case 10 uses test functions with allocator constructor
  /* case 10: {
    // --------------------------------------------------------------------
    // REMOVEALL TEST
    //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         removeAllTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
  ...
  // UNIMPLEMENTED: Case 9 uses test functions with allocator constructor
  /* case 9: {
    // --------------------------------------------------------------------
    // findR test
    //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         findRTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
  ...
  // UNIMPLEMENTED: Case 8 uses test functions with allocator constructor
  /* case 8: {
    // --------------------------------------------------------------------
    // FIND TEST
    //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         findTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
  ...
  case 7: {
    ...
    // --------------------------------------------------------------------
    if (verbose)
      cout << endl << "timer test" << endl << "==========" << endl;

    typedef bdlcc_compat::SkipList<bsls::TimeInterval, bsl::function<void()>>
        EventTimeQueue;
  ...
  // UNIMPLEMENTED: Case 6 uses test functions with allocator constructor
  /* case 6: {
    // --------------------------------------------------------------------
    // ITERATION TEST
    //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         iterationTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         updateTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
    ...
    */
    ...
    // UNIMPLEMENTED: Case 4 uses test functions with allocator constructor
    /* case 4: {
      // --------------------------------------------------------------------
      // NEWFRONT TEST
      //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         newFrontTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
    ...
    // UNIMPLEMENTED: Case 3 uses test functions with allocator constructor
    /* case 3: {
      // --------------------------------------------------------------------
      // Allocation Test
      //
...
// UNIMPLEMENTED: valueSemanticTest() -         U_RUN_EACH_TYPE(TestDriver,
// UNIMPLEMENTED: valueSemanticTest() -                         allocationTest,
// UNIMPLEMENTED: valueSemanticTest() - U_TEST_TYPES_REGULAR);
    ...
    // UNIMPLEMENTED: Case 2 - STM incompatibility -       case 2: {
    // UNIMPLEMENTED: Case 2 - STM incompatibility -         //
    // --------------------------------------------------------------------
    // UNIMPLEMENTED: Case 2 - STM incompatibility -         // USAGE EXAMPLE
    // UNIMPLEMENTED: Case 2 - STM incompatibility -         //
    // --------------------------------------------------------------------
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -         if (verbose) cout
    // << endl UNIMPLEMENTED: Case 2 - STM incompatibility - << "Usage Example"
    // << endl UNIMPLEMENTED: Case 2 - STM incompatibility - << "============="
    // << endl; UNIMPLEMENTED: Case 2 - STM incompatibility - UNIMPLEMENTED:
    // Case 2 - STM incompatibility -         ASSERT(0 ==
    // completionGuard.guard(bsls::TimeInterval(270, 0), UNIMPLEMENTED: Case 2 -
    // STM incompatibility - bsl::format("case {}", test))); UNIMPLEMENTED: Case
    // 2 - STM incompatibility - UNIMPLEMENTED: Case 2 - STM incompatibility -
    // using namespace USAGE; UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -         {
    // UNIMPLEMENTED: Case 2 - STM incompatibility - // ```
    // UNIMPLEMENTED: Case 2 - STM incompatibility - // Now, in `main`, verify
    // that the scheduler executes events when expected: UNIMPLEMENTED: Case 2 -
    // STM incompatibility - // ``` UNIMPLEMENTED: Case 2 - STM incompatibility
    // -     SimpleScheduler      scheduler; UNIMPLEMENTED: Case 2 - STM
    // incompatibility - UNIMPLEMENTED: Case 2 - STM incompatibility -
    // bsl::vector<int>     values; UNIMPLEMENTED: Case 2 - STM incompatibility
    // - UNIMPLEMENTED: Case 2 - STM incompatibility -     const bdlt::Datetime
    // start = bdlt::CurrentTime::utc(); UNIMPLEMENTED: Case 2 - STM
    // incompatibility -     bdlt::Datetime       scheduleTime; UNIMPLEMENTED:
    // Case 2 - STM incompatibility - // ``` UNIMPLEMENTED: Case 2 - STM
    // incompatibility - // Add events out of sequence and ensure they are
    // executed in the proper order. UNIMPLEMENTED: Case 2 - STM incompatibility
    // - // ``` UNIMPLEMENTED: Case 2 - STM incompatibility - if (veryVerbose)
    // cout << "Start:      " << start << endl; UNIMPLEMENTED: Case 2 - STM
    // incompatibility - UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduleTime = start; UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduleTime.addMilliseconds(2250); UNIMPLEMENTED: Case 2 - STM
    // incompatibility - if (veryVerbose) cout << "scheduling: " << scheduleTime
    // << endl; UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduler.scheduleEvent(bdlf::BindUtil::bind(&pushBackWrapper, &values,
    // 2), UNIMPLEMENTED: Case 2 - STM incompatibility - scheduleTime);
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -     scheduleTime = start;
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduleTime.addMilliseconds(750); UNIMPLEMENTED: Case 2 - STM
    // incompatibility - if (veryVerbose) cout << "scheduling: " << scheduleTime
    // << endl; UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduler.scheduleEvent(bdlf::BindUtil::bind(&pushBackWrapper, &values,
    // 0), UNIMPLEMENTED: Case 2 - STM incompatibility - scheduleTime);
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -     scheduleTime = start;
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduleTime.addMilliseconds(1500); UNIMPLEMENTED: Case 2 - STM
    // incompatibility - if (veryVerbose) cout << "scheduling: " << scheduleTime
    // << endl; UNIMPLEMENTED: Case 2 - STM incompatibility -
    // scheduler.scheduleEvent(bdlf::BindUtil::bind(&pushBackWrapper, &values,
    // 1), UNIMPLEMENTED: Case 2 - STM incompatibility - scheduleTime);
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -     ASSERT(values.empty());
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -     scheduler.drain();
    // UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility -     bdlt::Datetime finish =
    // bdlt::CurrentTime::utc(); UNIMPLEMENTED: Case 2 - STM incompatibility -
    // if (veryVerbose) cout << "Finish:     " << finish << endl; UNIMPLEMENTED:
    // Case 2 - STM incompatibility - UNIMPLEMENTED: Case 2 - STM
    // incompatibility -     ASSERT(3 == values.size()); UNIMPLEMENTED: Case 2 -
    // STM incompatibility -     ASSERT(0 == values[0]); UNIMPLEMENTED: Case 2 -
    // STM incompatibility -     ASSERT(1 == values[1]); UNIMPLEMENTED: Case 2 -
    // STM incompatibility -     ASSERT(2 == values[2]); UNIMPLEMENTED: Case 2 -
    // STM incompatibility - UNIMPLEMENTED: Case 2 - STM incompatibility - const
    // double elapsed = bdlt::IntervalConversionUtil::convertToTimeInterval(
    // UNIMPLEMENTED: Case 2 - STM incompatibility - finish -
    // start).totalSecondsAsDouble(); UNIMPLEMENTED: Case 2 - STM
    // incompatibility - UNIMPLEMENTED: Case 2 - STM incompatibility -
    // ASSERT(2.25 <= elapsed); UNIMPLEMENTED: Case 2 - STM incompatibility -
    // ASSERT(elapsed < 2.75); UNIMPLEMENTED: Case 2 - STM incompatibility -
    // UNIMPLEMENTED: Case 2 - STM incompatibility - if (veryVerbose) cout <<
    // "Elapsed: " << elapsed << " seconds\n"; UNIMPLEMENTED: Case 2 - STM
    // incompatibility - // ``` UNIMPLEMENTED: Case 2 - STM incompatibility - //
    // Note that the destructor of `scheduler` will call `stop()`.
    // UNIMPLEMENTED: Case 2 - STM incompatibility - // ```
    // UNIMPLEMENTED: Case 2 - STM incompatibility -         }
    // UNIMPLEMENTED: Case 2 - STM incompatibility -       } break;
  case 1: {
    // --------------------------------------------------------------------
    // BREATHING TEST
    // --------------------------------------------------------------------
    ...
    {
      typedef bdlcc_compat::SkipList<int, const char *> Obj;
      ...
      // UNIMPLEMENTED: popFrontRaw, ->data(), releaseReferenceRaw, ->key() not
      // supported
      /*
      Obj::Pair *h = 0;
      ...
      */
      ...
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      //             typedef bdlcc_compat::SkipList<int, bsl::string> Obj2;
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      ...
      //
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      //             // UNIMPLEMENTED: popFrontRaw, ->data(), ->key(),
      //             releaseReferenceRaw not supported
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      //             /*
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      //             Obj2::Pair *h2 = 0;
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      //             int ret = mX2.popFrontRaw(&h2);
      // UNIMPLEMENTED: bsl::string is not nothrow swappable for STM
      ...
      // UNIMPLEMENTED: bsl::string section -             ret =
      // mX2.popFront(&h2h); UNIMPLEMENTED: bsl::string section - ASSERT(ret ==
      // Obj::e_NOT_FOUND);
    }
  } break;
  case -105: {
...
#error "unrecognized platform"
    ...
    */
    ...
    for (int ii = 0; ii < k_NUM_RUNS; ii += k_DISPLAY_DELTA) {
      ...
      cout << userTimes[ii] << ',' << systemTimes[ii] << ',' << wallTimes[ii]
           << endl;
    ...
    for (int vecI = 0; vecI < 3; ++vecI) {
      const bsl::vector<double> &vec = 0 == vecI   ? userTimes
                                       : 1 == vecI ? systemTimes
                                                   : wallTimes;
  ...
  case -101: {
    ...
    // --------------------------------------------------------------------
    // UNIMPLEMENTED: run() uses Raw methods
