...
Definition readOrRetry {A} (tv : TVar A) (ok : A -> bool) : itree iSTM A :=
  x <- readTVar tv ;;
...
Definition stm_basic_counter (_ : unit) : itree iSTM nat :=
  c <- newTVar 0 ;;
  writeTVar c 1 ;;
  readTVar c.

Definition io_basic_counter : itree iIO nat :=
  atomically (stm_basic_counter tt).

(* 2) Increment test: write = read + 1; returns x+1 *)
Definition stm_inc (x : nat) : itree iSTM nat :=
  c <- newTVar x ;;
  modifyTVar c (fun n => S n) ;;
  readTVar c.

Definition io_inc (x : nat) : itree iIO nat :=
  atomically (stm_inc x).

(* 3) Read-your-own-writes and sequencing; returns x + (x) = 2x *)
Definition stm_add_self (x : nat) : itree iSTM nat :=
  c <- newTVar x ;;
  v <- readTVar c ;;
  writeTVar c (v + x) ;;
  readTVar c.

Definition io_add_self (x : nat) : itree iIO nat :=
  atomically (stm_add_self x).

(* 4) A small queue modeled as list nat inside a TVar *)

(* push at tail *)
Definition stm_enqueue (q : TVar (list nat)) (x : nat) : itree iSTM unit :=
  xs <- readTVar q ;;
  writeTVar q (xs ++ [x]).

(* pop from head; retry if empty *)
Definition stm_dequeue (q : TVar (list nat)) : itree iSTM nat :=
  xs <- readTVar q ;;
  match xs with
  | []      => retry
...
Definition stm_tryDequeue (q : TVar (list nat)) (dflt : nat) : itree iSTM nat :=
  orElse (stm_dequeue q) (Ret dflt).

(* smoke test: enqueue then dequeue must return the enqueued element *)
Definition stm_queue_roundtrip (x : nat) : itree iSTM nat :=
  q <- newTVar ([] : list nat) ;;
  stm_enqueue q x ;;
  stm_dequeue q.

Definition io_queue_roundtrip (x : nat) : itree iIO nat :=
  atomically (stm_queue_roundtrip x).

(* 5) orElse + retry behavior *)
(* First branch retries on empty; second returns a constant 42 *)
Definition stm_orElse_retry_example (_ : unit) : itree iSTM nat :=
  q <- newTVar ([] : list nat) ;;
  orElse (stm_dequeue q) (Ret 42).

Definition io_orElse_retry_example : itree iIO nat :=
...
Definition stm_wait_at_least (c : TVar nat) (k : nat) : itree iSTM nat :=
  readOrRetry c (fun n => Nat.leb k n).

(* With orElse, provide a default if not ready *)
Definition stm_wait_or_default (c : TVar nat) (k dflt : nat) : itree iSTM nat :=
...
Definition stm_transfer (a b : TVar nat) (amt : nat) : itree iSTM unit :=
  va <- readTVar a ;;
  vb <- readTVar b ;;
...
Definition stm_transfer_test (a0 b0 amt : nat) : itree iSTM (prod nat nat) :=
  a  <- newTVar a0 ;;
  b  <- newTVar b0 ;;
...
Definition io_transfer_test (a0 b0 amt : nat) : itree iIO (prod nat nat) :=
  atomically (stm_transfer_test a0 b0 amt).

(* 8) Polymorphic retry witness (type-check only) *)
Definition stm_retry_nat   : itree iSTM nat   := retry.
Definition stm_retry_list  : itree iSTM (list nat) := retry.

(* 9) Combining orElse with mismatched branches is rejected by typing;
   this one typechecks because both are itree iSTM nat *)
Definition stm_orElse_ok : itree iSTM nat :=
  orElse (Ret 1) (Ret 2).

(* 10) A tiny “modify then read” utility test *)
Definition stm_modify_read (x : nat) : itree iSTM nat :=
  c  <- newTVar x ;;
  _  <- modifyTVar c (fun n => n + 3) ;;
  readTVar c.

Definition io_modify_read (x : nat) : itree iIO nat :=
