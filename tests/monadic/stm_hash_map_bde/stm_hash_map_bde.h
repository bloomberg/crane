...
V>((*(this)).CHT::cht_eqb, k,
xs);
auto _cs = p.first;
if (_cs.has_value()) { V _x = *_cs; stm::writeTVar(b, p.second);
...
bsl::shared_ptr<CHT<K, V>> _self = bsl::make_shared<CHT<K,
V>>(*(this));
...
F0>
__attribute__((pure)) static bsl::optional<T2> assoc_lookup(F0&& eqb,
const T1 k,
...
T1 k_ = d_a0.first; T2 v = d_a0.second; if (eqb(k,
k_)) { return bsl::make_optional<T2>(v); } else { return CHT<int, int>::template assoc_lookup<T1,
T2>(eqb, k, *(d_a1)); }
}}template <typename T1, typename T2, MapsTo<bool, T1, T1>
F0>
__attribute__((pure)) static List<bsl::pair<T1, T2>> assoc_insert_or_replace(F0&& eqb,
...
const auto& [d_a0, d_a1] = bsl::get<typename List<bsl::pair<T1, T2>>::Cons>(xs.v());
T1 k_ = d_a0.first; T2 v_ = d_a0.second; if (eqb(k,
k_)) { return List<bsl::pair<T1, T2>>::cons(bsl::make_pair(k, v),
*(d_a1)); } else { return List<bsl::pair<T1, T2>>::cons(bsl::make_pair(k_, v_),
CHT<int, int>::template assoc_insert_or_replace<T1, T2>(eqb, k, v,
*(d_a1))); }
}}template <typename T1, typename T2, MapsTo<bool, T1, T1>
F0>
__attribute__((pure)) static bsl::pair<bsl::optional<T2>, List<bsl::pair<T1, T2>>> assoc_remove(F0&& eqb,
const T1 k,
List<bsl::pair<T1, T2>> xs){if (bsl::holds_alternative<typename List<bsl::pair<T1, T2>>::Nil>(xs.v())) {
...
T2>(eqb, k,
*(d_a1));
return bsl::make_pair(q.first, List<bsl::pair<T1, T2>>::cons(bsl::make_pair(k_, v_),
q.second)); }
}}template <typename T1, typename
T2>
static bsl::vector<stm::TVar<List<bsl::pair<T1, T2>>>> mk_buckets(const int64_t num){bsl::vector<stm::TVar<List<bsl::pair<T1, T2>>>> buckets = {};
bsl::function<bsl::vector<stm::TVar<List<bsl::pair<T1, T2>>>>(unsigned int)> f;
f = [&](unsigned int
...
MapsTo<bool, T1, T1> F0, MapsTo<int64_t, T1> F1>
static CHT<T1, T2> new_hash(F0&& eqb, F1&& hash,
const int64_t requested){int64_t n = bsl::max<int64_t>(requested, 1);
