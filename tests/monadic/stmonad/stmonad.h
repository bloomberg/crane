...
template <typename T1, typename T2> Prod<Nat, Nat> newAndReadBoth() {
  Nat r1;
  r1 = Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::o())))));
  Nat r2;
  r2 = Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::o()))))));
  Nat x1 = r1;
  Nat x2 = r2;
  return Prod<Nat, Nat>::pair(std::move(x1), std::move(x2));
}

template <typename T1, typename T2> Nat tree_simp() {
  Nat v;
  v = Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::o())))));
  return v;
}

template <typename T1, typename T2> Nat tree_simp_another() {
  Nat v;
  v = Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::o())))));
  v = Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::s(Nat::o()))))));
  Nat val = v;
  return val;
}
