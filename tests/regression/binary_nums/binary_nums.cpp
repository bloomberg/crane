...
std::shared_ptr<N> Pos::pred_N(const std::shared_ptr<Positive> &x) {
  return std::visit(
      Overloaded{[](const typename Positive::XI &_args) -> std::shared_ptr<N> {
                 ...
                 [](const typename Positive::XH &) -> std::shared_ptr<N> {
                   return N::n0();
                 }},
      x->v());
}
...
Pos::succ_double_mask(const std::shared_ptr<Pos::mask> &x) {
  return std::visit(
      Overloaded{
          [](const typename Pos::mask::IsNul &) -> std::shared_ptr<Pos::mask> {
          ...
          [](const typename Pos::mask::IsNeg &) -> std::shared_ptr<Pos::mask> {
            return mask::isneg();
          }},
      x->v());
}
...
Pos::double_pred_mask(const std::shared_ptr<Positive> &x) {
  return std::visit(
      Overloaded{
          ...
          [](const typename Positive::XH &) -> std::shared_ptr<Pos::mask> {
            return mask::isnul();
          }},
      x->v());
}
          ...
          [&](const typename Positive::XH &) -> std::shared_ptr<Pos::mask> {
            return std::visit(
                Overloaded{
                    ...
                    [](const auto &) -> std::shared_ptr<Pos::mask> {
                      return mask::isneg();
                    }},
                y->v());
          }},
...
Pos::compare_cont(const Comparison r, const std::shared_ptr<Positive> &x,
  ...
  return std::visit(
      Overloaded{
          [&](const typename Positive::XI &_args) -> Comparison {
            return std::visit(
                Overloaded{
                    ...
                    [](const typename Positive::XH &) -> Comparison {
          ...
          [&](const typename Positive::XH &) -> Comparison {
            return std::visit(
                Overloaded{[&](const typename Positive::XH &) -> Comparison {
                             return r;
                           },
...
std::shared_ptr<N> BinNat::sub(std::shared_ptr<N> n,
  ...
  if (n.use_count() == 1 && n->v().index() == 0) {
    ...
    return std::visit(
        Overloaded{
            [](const typename N::N0 &) -> std::shared_ptr<N> {
              return N::n0();
            },
...
__attribute__((pure)) Comparison BinNat::compare(const std::shared_ptr<N> &n,
                                                 const std::shared_ptr<N> &m) {
  return std::visit(
      Overloaded{[&](const typename N::N0 &) -> Comparison {
                   return std::visit(
                       Overloaded{[](const typename N::N0 &) -> Comparison {
                                    return Comparison::e_EQ;
                                  },
                                  [](const typename N::Npos &) -> Comparison {
                                    return Comparison::e_LT;
                                  }},
                       m->v());
                 },
                 [&](const typename N::Npos &_args) -> Comparison {
                   return std::visit(
                       Overloaded{
                           [](const typename N::N0 &) -> Comparison {
                             return Comparison::e_GT;
                           },
...
std::shared_ptr<N> BinNat::pred(const std::shared_ptr<N> &n) {
  return std::visit(
      Overloaded{
          [](const typename N::N0 &) -> std::shared_ptr<N> { return N::n0(); },
          [](const typename N::Npos &_args) -> std::shared_ptr<N> {
...
std::shared_ptr<N> BinNat::add(std::shared_ptr<N> n, std::shared_ptr<N> m) {
  return std::visit(
      Overloaded{
          [&](const typename N::N0 &) -> std::shared_ptr<N> { return m; },
          [&](const typename N::Npos &_args) -> std::shared_ptr<N> {
            if (m.use_count() == 1 && m->v().index() == 1) {
              ...
              return std::visit(
                  Overloaded{[&](const typename N::N0 &) -> std::shared_ptr<N> {
                               return n;
                             },
                             [&](const typename N::Npos &_args0)
                                 -> std::shared_ptr<N> {
                               return N::npos(
                                   Coq_Pos::add(_args.d_a0, _args0.d_a0));
                             }},
                  m->v());
            }
          }},
...
__attribute__((pure)) unsigned int BinNat::to_nat(const std::shared_ptr<N> &a) {
  return std::visit(
      Overloaded{[](const typename N::N0 &) -> unsigned int { return 0u; },
  ...
  return std::visit(
      Overloaded{
          ...
          [&](const typename Z::Zpos &_args) -> std::shared_ptr<Z> {
            return std::visit(
                Overloaded{
                    [](const typename Z::Z0 &) -> std::shared_ptr<Z> {
                      return Z::z0();
                    },
...
__attribute__((pure)) Comparison BinInt::compare(const std::shared_ptr<Z> &x,
  ...
  return std::visit(
      Overloaded{
          [&](const typename Z::Z0 &) -> Comparison {
            return std::visit(
                Overloaded{[](const typename Z::Z0 &) -> Comparison {
                             return Comparison::e_EQ;
                           },
                           [](const typename Z::Zpos &) -> Comparison {
                             return Comparison::e_LT;
                           },
                           [](const typename Z::Zneg &) -> Comparison {
                             return Comparison::e_GT;
                           }},
                y->v());
          },
...
std::shared_ptr<Z> BinaryNums::z_sign(const std::shared_ptr<Z> &z) {
  return std::visit(
      Overloaded{
          [](const typename Z::Z0 &) -> std::shared_ptr<Z> { return Z::z0(); },
          [](const typename Z::Zpos &) -> std::shared_ptr<Z> {
            return Z::zpos(Positive::xh());
          },
          [](const typename Z::Zneg &) -> std::shared_ptr<Z> {
            return Z::zneg(Positive::xh());
          }},
      z->v());
}
