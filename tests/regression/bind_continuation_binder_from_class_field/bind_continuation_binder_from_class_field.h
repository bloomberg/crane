...
public:
  ...
  template <typename _U> Dv(const Dv<_U> &_other) {
    if (std::holds_alternative<typename Dv<_U>::DAddr>(_other.v())) {
      const auto &[a0] = std::get<typename Dv<_U>::DAddr>(_other.v());
      this->v_ = DAddr{a0};
    } else {
      const auto &[a0] = std::get<typename Dv<_U>::DNum>(_other.v());
      this->v_ = DNum{a0};
    }
  }
