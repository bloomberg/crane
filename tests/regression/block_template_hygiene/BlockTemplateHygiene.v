...
Module BlockTemplateHygiene.
  ...
  Definition same_name_twice : itree iIO string :=
    s <- get_line ;;
    s <- get_line ;;
    Ret (cat s " (second)").
  ...
  Definition same_name_thrice : itree iIO string :=
    s <- get_line ;;
    s <- get_line ;;
    s <- get_line ;;
    Ret (cat s " (third)").
  ...
  Definition shadow_internal_name : itree iIO string :=
    s <- get_line ;;
    Ret (cat s "!").
  ...
  Definition interleaved_templates : itree iIO string :=
    a <- get_line ;;
    print a ;;
    b <- get_line ;;
    print_endline b ;;
  ...
  (** Test 7: Block template result used as an argument to another call.
      ...
      block expansion. *)
  Definition result_in_expr : itree iIO unit :=
    name <- get_line ;;
  ...
  (** Test 8: Block template in a let-binding context (non-monadic).
      ...
      template result. *)
  Definition let_after_block : itree iIO string :=
    first <- get_line ;;
    last <- get_line ;;
  ...
  (** Test 9: get_line bound to the name [result] to stress-test that
      ...
      expansion logic. *)
  Definition bind_named_result : itree iIO string :=
    result <- get_line ;;
    Ret (cat result "!").
  ...
  Definition bind_named_underscore_r : itree iIO string :=
    _r <- get_line ;;
    Ret (cat _r "!").
  ...
  Definition expr_position_iife : itree iIO string :=
    s <- get_line ;;
    Ret s.
