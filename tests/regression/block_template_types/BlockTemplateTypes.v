...
Module BlockTemplateTypes.
  ...
  Definition test_read_nat : itree ioE nat :=
    n <- read_nat ;;
    Ret (n + 1).

  (** %result inferred as [bool], with [unsigned int] in same scope. *)
  Definition test_is_positive : itree ioE string :=
    n <- read_nat ;;
    b <- is_positive n ;;
  ...
  Definition test_parse_nat : itree ioE nat :=
    s <- get_line ;;
    n <- parse_nat s ;;
    Ret (n + 2).
  ...
  Definition test_mixed : itree ioE string :=
    name <- get_line ;;
    n <- read_nat ;;
    b <- is_positive n ;;
  ...
  Definition test_nat_arith : itree ioE nat :=
    a <- read_nat ;;
    b <- read_nat ;;
    Ret (a + b).
