  ...
  public:
    ...
    __attribute__((pure)) fin clone() const {
      ...
      if (std::holds_alternative<FZ>(_sv.v())) {
        ...
        return fin(FZ{[](auto &&__v) -> unsigned int {
          if constexpr (
        ...
        return fin(
            FS{[](auto &&__v) -> unsigned int {
               ...
               d_a1 ? std::make_unique<DepElim::fin>(d_a1->clone()) : nullptr});
      }
    }
  ...
  public:
    ...
    __attribute__((pure)) avail clone() const {
      ...
      if (std::holds_alternative<Present>(_sv.v())) {
        ...
        return avail(Present{[](auto &&__v) -> unsigned int {
          if constexpr (
              requires { __v ? 0 : 0; } && requires { *__v; } &&
              requires { __v->clone(); } && requires { __v.get(); }) {
            using _E = std::remove_cvref_t<decltype(*__v)>;
