...
(** Runtime bad_any_cast repro (now fixed). Mirrors Crane's codegen for
    ...
    destructured from an erased tuple (so it is std::any) -- was emitted by Crane
    as
      std::make_pair(v, std::monostate{})
    i.e. a [std::pair<std::any, std::monostate>]: the [tt : unit] component was
    left as a raw [std::monostate{}] instead of being erased to
    [std::any(std::monostate{})]. So the value's dynamic type was
    ...
    for [ConstRef]s, so the [syms_semty -> tuple -> std::any] chain resolves and
