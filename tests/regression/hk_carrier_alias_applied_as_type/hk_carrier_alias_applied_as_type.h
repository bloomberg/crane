...
List<T1<std::any>> TFunctor_list_of(std::type_identity_t<TFunctor<T1>> h,
                                    F1 &&f, const List<T1<std::any>> &l) {
...
modul<std::any, T1<std::any>> TFunctor_modul(
    std::type_identity_t<TFunctor<T1>>,
    std::type_identity_t<TFunctor<_crane_carrier_tch<T1>::template c>> h0,
    F2 &&f, const modul<std::any, T1<std::any>> &m) {
  return modul<std::any, T1<std::any>>{tfmap<List>(
      [=]() mutable {
        return [=](std::function<std::any(std::any)> _x0,
                   List<two<std::any, T1<std::any>>> _x1) mutable
                   -> List<two<std::any, T1<std::any>>> {
          return TFunctor_list_of<_crane_carrier_tch<T1>::template c>(h0, _x0,
                                                                      _x1);
        };
      }(),
      f, m.m_items)};
