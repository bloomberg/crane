#ifndef INCLUDED_HK_INSTANCE_BODY_TARG_UNDECLARED
#define INCLUDED_HK_INSTANCE_BODY_TARG_UNDECLARED
...
TFunctor_option(std::type_identity_t<TFunctor<T1>> h, F1 &&f,
  ...
  if (ot.has_value()) {
    const auto &t = *ot;
    return std::make_optional<T1<std::any>>(tfmap<T1, std::any>(h, f, t));
  } else {
