...
#include <functional>
#include <iostream>
#include <jcn_condition_carry_clear_gate.h>
#include <memory>
#include <optional>
#include <stdexcept>
#include <string>
...
unsigned int JcnConditionCarryClearGate::acc(
    const std::shared_ptr<JcnConditionCarryClearGate::state> &s) {
  return s->acc;
}

bool JcnConditionCarryClearGate::carry(
    const std::shared_ptr<JcnConditionCarryClearGate::state> &s) {
  return s->carry;
}

bool JcnConditionCarryClearGate::test_pin(
    const std::shared_ptr<JcnConditionCarryClearGate::state> &s) {
  return s->test_pin;
}

bool JcnConditionCarryClearGate::jcn_condition(
    const std::shared_ptr<JcnConditionCarryClearGate::state> &s,
