...
#include <functional>
#include <iostream>
#include <jcn_condition_nonzero_gate.h>
#include <memory>
#include <optional>
#include <stdexcept>
#include <string>
...
unsigned int JcnConditionNonzeroGate::acc(
    const std::shared_ptr<JcnConditionNonzeroGate::state> &s) {
  return s->acc;
}

bool JcnConditionNonzeroGate::carry(
    const std::shared_ptr<JcnConditionNonzeroGate::state> &s) {
  return s->carry;
}

bool JcnConditionNonzeroGate::test_pin(
    const std::shared_ptr<JcnConditionNonzeroGate::state> &s) {
  return s->test_pin;
}

bool JcnConditionNonzeroGate::jcn_condition(
    const std::shared_ptr<JcnConditionNonzeroGate::state> &s,
