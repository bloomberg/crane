...
#include <functional>
#include <iostream>
#include <jcn_condition_test_low.h>
#include <memory>
#include <optional>
#include <stdexcept>
#include <string>
...
unsigned int
JcnConditionTestLow::acc(const std::shared_ptr<JcnConditionTestLow::state> &s) {
  return s->acc;
}

bool JcnConditionTestLow::carry(
    const std::shared_ptr<JcnConditionTestLow::state> &s) {
  return s->carry;
}

bool JcnConditionTestLow::test_pin(
    const std::shared_ptr<JcnConditionTestLow::state> &s) {
  return s->test_pin;
}

bool JcnConditionTestLow::jcn_condition(
    const std::shared_ptr<JcnConditionTestLow::state> &s,
