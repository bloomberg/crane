...
#include <functional>
#include <iostream>
#include <jcn_condition_zero_gate.h>
#include <memory>
#include <optional>
#include <stdexcept>
#include <string>
...
unsigned int JcnConditionZeroGate::acc(
    const std::shared_ptr<JcnConditionZeroGate::state> &s) {
  return s->acc;
}

bool JcnConditionZeroGate::carry(
    const std::shared_ptr<JcnConditionZeroGate::state> &s) {
  return s->carry;
}

bool JcnConditionZeroGate::test_pin(
    const std::shared_ptr<JcnConditionZeroGate::state> &s) {
  return s->test_pin;
}

bool JcnConditionZeroGate::jcn_condition(
    const std::shared_ptr<JcnConditionZeroGate::state> &s,
