...
bool JcnOps::jcn_condition(const JcnOps::state &s, unsigned int cond) {
...
unsigned int JcnOps::addr12_of_nat(unsigned int n) {
...
unsigned int JcnOps::page_of(unsigned int p) { return (256u ? p / 256u : 0); }

unsigned int JcnOps::page_base(unsigned int p) { return (page_of(p) * 256u); }

unsigned int JcnOps::base_for_next2(const JcnOps::state &s) {
  return page_base(pc_inc2(s));
}

unsigned int JcnOps::branch_target(const JcnOps::state &s, unsigned int cond,
                                   unsigned int off) {
