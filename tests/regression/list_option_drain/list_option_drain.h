#ifndef INCLUDED_LIST_OPTION_DRAIN
#define INCLUDED_LIST_OPTION_DRAIN
...
struct ListOptionDrain {
  struct t {
    // TYPES
    struct Node {
      uint64_t a0;
      std::shared_ptr<List<std::optional<t>>> a1;
    };
  ...
  public:
    ...
    static t node(uint64_t a0, List<std::optional<t>> a1) {
      return t(
          Node{a0, std::make_shared<List<std::optional<t>>>(std::move(a1))});
    }
  ...
  static inline const t empty =
      t::node(UINT64_C(0), List<std::optional<t>>::nil());
};

#endif // INCLUDED_LIST_OPTION_DRAIN
