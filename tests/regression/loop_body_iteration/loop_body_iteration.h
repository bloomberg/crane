...
struct LoopBodyIteration {
  struct state {
    ...
    // ACCESSORS
    __attribute__((pure)) state clone() const { return state{(*(this)).regs_}; }
  };
  ...
  __attribute__((pure)) static unsigned int get_reg0(const state &s);
  __attribute__((pure)) static state count_loop_body(const state &s);
  __attribute__((pure)) static state iterate_body(const unsigned int &n,
                                                  state s);
