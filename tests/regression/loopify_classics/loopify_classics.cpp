...
LoopifyClassics::binomial_fuel(const unsigned int fuel, const unsigned int n,
  ...
  unsigned int _result{};
  std::vector<_Frame> _stack;
  _stack.push_back(_Enter{k, n, fuel});
  while (!_stack.empty()) {
    _Frame _frame = std::move(_stack.back());
    _stack.pop_back();
    std::visit(Overloaded{[&](_Enter _f) {
                          ...
                          [&](_After _f) {
                            _stack.push_back(_Combine{_result});
                            _stack.push_back(_Enter{_f._a0, _f._a1, _f._a2});
                          },
                          [&](_Combine _f) { _result = (_result + _f._left); }},
               _frame);
  }
...
LoopifyClassics::pascal_fuel(const unsigned int fuel, const unsigned int row,
  ...
  unsigned int _result{};
  std::vector<_Frame> _stack;
  _stack.push_back(_Enter{col, row, fuel});
  while (!_stack.empty()) {
    _Frame _frame = std::move(_stack.back());
    _stack.pop_back();
    std::visit(
  ...
  while (!_stack.empty()) {
    ...
    std::visit(Overloaded{[&](_Enter _f) {
                            ...
                            if (exp <= 0) {
                              ...
                              _stack.push_back(_Call1{base});
                              _stack.push_back(_Enter{exp_});
                            }
                          },
                          [&](_Call1 _f) { _result = (_f._s0 * _result); }},
               _frame);
