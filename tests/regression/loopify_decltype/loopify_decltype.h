#ifndef INCLUDED_LOOPIFY_DECLTYPE
...
struct LoopifyDecltype {
  ...
  /// contribution.
  __attribute__((pure)) static unsigned int count_true(const List<bool> &xs);
  ...
  struct item {
    ...
    __attribute__((pure)) item clone() const {
      return item{clone_as_value<bool>((*(this)).item_flag),
                  clone_as_value<unsigned int>((*(this)).item_val)};
    }
  };

  __attribute__((pure)) static unsigned int sum_flagged(const List<item> &xs);
};
