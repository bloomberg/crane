    ...
    if (std::holds_alternative<_Enter>(_frame)) {
      ...
      if (std::holds_alternative<typename LoopifyMoreTrees::tree::Leaf>(
        ...
        if (std::holds_alternative<typename LoopifyMoreTrees::tree::Leaf>(
          ...
          _stack.emplace_back(_After2{d_a00.get(), d_a0.get()});
...
LoopifyMoreTrees::insert_bst(unsigned int x, const LoopifyMoreTrees::tree &t) {
  ...
  while (!_stack.empty()) {
    ...
    if (std::holds_alternative<_Enter>(_frame)) {
      ...
      _result = tree::node(_result, _f.d_a1, _f._s0);
    } else {
      auto _f = std::move(std::get<_Resume2>(_frame));
      _result = tree::node(_f._s1, _f.d_a1, _result);
    }
  }
  return _result;
}
  ...
  while (!_stack.empty()) {
    ...
    if (std::holds_alternative<_Enter>(_frame)) {
      ...
      if (std::holds_alternative<typename List<LoopifyMoreTrees::tree>::Nil>(
        ...
        _stack.emplace_back(_Resume1{tree_children(d_a0)});
        _stack.emplace_back(_Enter{d_a1.get()});
      }
    } else {
      auto _f = std::move(std::get<_Resume1>(_frame));
