...
List<unsigned int> LoopifySearchOpt::longest_run_fuel(
    unsigned int fuel, List<unsigned int> current, List<unsigned int> best,
    ...
    if (_loop_fuel <= 0) {
      ...
      if (std::holds_alternative<typename List<unsigned int>::Nil>(
        ...
        if (std::holds_alternative<typename List<unsigned int>::Nil>(
          ...
          auto &[a00, a10] = std::get<typename List<unsigned int>::Cons>(
              _loop_current.v_mut());
          if (a0 == std::move(a00)) {
            _loop_l = a1.get();
            _loop_current = List<unsigned int>::cons(a0, _loop_current);
            _loop_fuel = fuel_;
          } else {
            unsigned int len_curr = _loop_current.length();
            unsigned int len_best = _loop_best.length();
...
unsigned int LoopifySearchOpt::knapsack_fuel(
  ...
  /// call.
  struct _After2 {
    const List<std::pair<unsigned int, unsigned int>> *a1;
    unsigned int capacity;
    unsigned int fuel_;
    unsigned int value;
  };
  ...
  while (!_stack.empty()) {
    ...
    if (std::holds_alternative<_Enter>(_frame)) {
      ...
      unsigned int capacity = _f.capacity;
...
bool LoopifySearchOpt::subset_sum_fuel(
    unsigned int fuel, unsigned int target,
...
bool LoopifySearchOpt::subset_sum(unsigned int target,
                                  const List<unsigned int> &l) {
...
bool LoopifySearchOpt::binary_search_fuel(unsigned int fuel,
                                          unsigned int target,
                                          const List<unsigned int> &l) {
  bool _result;
    ...
    if (_loop_fuel <= 0) {
      ...
      if (std::holds_alternative<typename List<unsigned int>::Nil>(
        ...
        if (len <= 1u) {
          ...
          auto take_impl =
              [](auto &_self_take, unsigned int n,
            ...
            if (n <= 0) {
              ...
              if (std::holds_alternative<typename List<unsigned int>::Nil>(
                ...
                return List<unsigned int>::cons(
                    a03, _self_take(_self_take, n_, *a13));
              }
            }
          };
...
bool LoopifySearchOpt::binary_search(unsigned int target,
                                     const List<unsigned int> &l) {
