...
LoopifySorting::partition(const unsigned int pivot,
  ...
  while (!_stack.empty()) {
    ...
    if (std::holds_alternative<_Enter>(_frame)) {
      ...
      if (_m.d_a0 <= pivot) {
        _result = std::make_pair(List<unsigned int>::cons(_m.d_a0, lo), hi);
      } else {
        _result = std::make_pair(lo, List<unsigned int>::cons(_m.d_a0, hi));
      }
    }
  }
  return _result;
...
LoopifySorting::is_sorted(const std::shared_ptr<List<unsigned int>> &l) {
  if (std::holds_alternative<typename List<unsigned int>::Nil>(l->v())) {
    ...
    return is_sorted_aux(_m.d_a0, _m.d_a1);
  }
}
