...
LoopifyTail::member(const unsigned int &x,
  ...
  bool _result;
  LoopifyTail::list<unsigned int> _loop_l = l;
  while (true) {
    if (std::holds_alternative<typename LoopifyTail::list<unsigned int>::Nil>(
      ...
      const auto &[d_a0, d_a1] =
          std::get<typename LoopifyTail::list<unsigned int>::Cons>(_loop_l.v());
      if (x == d_a0) {
        _result = true;
        break;
      } else {
  ...
  while (true) {
    if (std::holds_alternative<typename LoopifyTail::list<unsigned int>::Nil>(
      ...
      if (_loop_n == 0u) {
        ...
        LoopifyTail::list<unsigned int> _next_l = *(d_a1);
...
__attribute__((pure)) unsigned int LoopifyTail::lookup(
  ...
  unsigned int _result;
  LoopifyTail::list<std::pair<unsigned int, unsigned int>> _loop_l = l;
  while (true) {
