...
int main() {
  using List = LoopifyTmc::list<uint64_t>;

  // Build test lists
  auto l3 = from_vec<uint64_t>({1, 2, 3});
  auto l5 = from_vec<uint64_t>({4, 5, 6, 7, 8});
  auto empty = List::nil();
  ...
  // ===== map =====
  {
    auto doubled = LoopifyTmc::map<uint64_t, uint64_t>(
        [](uint64_t x) { return x * 2; }, l3);
    auto v = to_vec(doubled);
    ASSERT(v.size() == 3);
    ASSERT(v[0] == 2 && v[1] == 4 && v[2] == 6);

    auto empty_map = LoopifyTmc::map<uint64_t, uint64_t>(
        [](uint64_t x) { return x; }, empty);
  ...
  // ===== filter =====
  {
    auto evens = LoopifyTmc::filter(
        [](uint64_t x) { return x % 2 == 0; }, l5);
    auto v = to_vec(evens);
    ASSERT(v.size() == 3); // 4, 6, 8
    ASSERT(v[0] == 4 && v[1] == 6 && v[2] == 8);

    // filter keeping all
    auto all = LoopifyTmc::filter([](uint64_t) { return true; }, l3);
    ASSERT(to_vec(all) == to_vec(l3));

    // filter keeping none
    auto none = LoopifyTmc::filter([](uint64_t) { return false; }, l3);
  ...
  // ===== zip_with =====
  {
    auto result = LoopifyTmc::zip_with<uint64_t, uint64_t, uint64_t>(
        [](uint64_t a, uint64_t b) { return a + b; }, l3, l5);
    auto v = to_vec(result);
  ...
  // ===== Large-input test: verify no stack overflow =====
  {
    ...
    // map on large list
    auto mapped = LoopifyTmc::map<uint64_t, uint64_t>(
        [](uint64_t x) { return x + 1; }, big);
    ASSERT(!to_vec(mapped).empty());

    // filter on large list (keep ~half)
    auto filtered = LoopifyTmc::filter(
        [](uint64_t x) { return x % 2 == 0; }, big);
