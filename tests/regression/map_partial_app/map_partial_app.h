...
struct MapPartialApp {
  ...
  public:
    ...
    __attribute__((pure)) tree clone() const {
      ...
      if (std::holds_alternative<Leaf>(_sv.v())) {
        ...
        return tree(
            Node{d_a0 ? std::make_unique<MapPartialApp::tree>(d_a0->clone())
                      : nullptr,
                 d_a1,
                 d_a2 ? std::make_unique<MapPartialApp::tree>(d_a2->clone())
                      : nullptr});
      }
    }
  ...
  __attribute__((pure)) static tree wrap(tree t, unsigned int v);
  /// Sum a list of nats.
  ...
  static inline const unsigned int map_two_closures = []() {
    return []() {
      ...
      return (sum_list(r1) + sum_list(r2));
    }();
  }();
};
