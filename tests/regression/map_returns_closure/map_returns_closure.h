#ifndef INCLUDED_MAP_RETURNS_CLOSURE
#define INCLUDED_MAP_RETURNS_CLOSURE
...
struct MapReturnsClosure {
  ...
  static uint64_t apply_all(const List<std::function<uint64_t(uint64_t)>> &fs,
                            uint64_t n);
