...
struct MemSafetyProbe18 {
  ...
  public:
    ...
    tree tree_from_tree() const {
      return tree::node(tree::node(*(this), 0u, tree::leaf()),
                        (*(this)).tree_sum(),
  ...
  public:
    ...
    template <typename _U> explicit mylist(const mylist<_U> &_other) {
      if (std::holds_alternative<typename mylist<_U>::Mynil>(_other.v())) {
        ...
        this->d_v_ =
            Mycons{t_A(d_a0),
                   d_a1 ? std::make_unique<MemSafetyProbe18::mylist<t_A>>(*d_a1)
                        : nullptr};
      }
    }
  ...
  static mylist<tree> build_tree_list(tree t, const unsigned int n);
