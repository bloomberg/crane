...
MemSafetyProbe23::sum_with_original(MemSafetyProbe23::tree t) {
  if (std::holds_alternative<typename MemSafetyProbe23::tree::Leaf>(
    ...
    std::pair<MemSafetyProbe23::tree, unsigned int> pl = sum_with_original(*a0);
    std::pair<MemSafetyProbe23::tree, unsigned int> pr = sum_with_original(*a2);
...
MemSafetyProbe23::dup_and_double(MemSafetyProbe23::tree t) {
  if (std::holds_alternative<typename MemSafetyProbe23::tree::Leaf>(
    ...
    return std::make_pair(
        std::move(t), tree::node(pl.second, (std::move(a1) * 2u), pr.second));
  }
}
...
MemSafetyProbe23::interleaved_ops(const MemSafetyProbe23::tree &t) {
  if (std::holds_alternative<typename MemSafetyProbe23::tree::Leaf>(t.v())) {
    ...
    std::pair<unsigned int, unsigned int> pl = interleaved_ops(*a0);
    std::pair<unsigned int, unsigned int> pr = interleaved_ops(*a2);
...
MemSafetyProbe23::annotate_sizes(const MemSafetyProbe23::tree &t) {
  if (std::holds_alternative<typename MemSafetyProbe23::tree::Leaf>(t.v())) {
    ...
    const auto &[a0, a1, a2] =
        std::get<typename MemSafetyProbe23::tree::Node>(t.v());
    unsigned int sl = tree_size(*a0);
    unsigned int sr = tree_size(*a2);
    std::pair<MemSafetyProbe23::tree, unsigned int> pl = annotate_sizes(*a0);
    std::pair<MemSafetyProbe23::tree, unsigned int> pr = annotate_sizes(*a2);
