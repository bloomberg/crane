...
struct MemSafetyProbe24 {
  ...
  public:
    ...
    std::pair<tree, uint64_t> tag_tree() const {
      ...
      /// _Cont_Node: saves [_self, a1, a2], resumes after recursive call, then
      /// processes rest.
      struct _Cont_Node {
        const tree *_self;
        uint64_t a1;
      ...
      /// _Cont_Node_1: saves [_self, a1, pl], resumes after recursive call,
      /// then processes rest.
      struct _Cont_Node_1 {
        const tree *_self;
        uint64_t a1;
      ...
      crane::small_vector<_Frame> _stack;
      _stack.emplace_back(_Enter{_self});
      /// Loopified tag_tree: _Enter -> _Cont_Node -> _Cont_Node_1.
      while (!_stack.empty()) {
        _Frame _frame = std::move(_stack.back());
        _stack.pop_back();
        if (std::holds_alternative<_Enter>(_frame)) {
          ...
          if (std::holds_alternative<typename tree::Leaf>(_sv.v())) {
            ...
            const auto &[a0, a1, a2] = std::get<typename tree::Node>(_sv.v());
            _stack.emplace_back(_Cont_Node{_self, a1, a2});
          ...
          _stack.emplace_back(_Cont_Node_1{_self, a1, std::move(pl)});
          _stack.emplace_back(_Enter{crane_raw(a2)});
        } else {
