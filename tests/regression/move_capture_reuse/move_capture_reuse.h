...
struct MoveCaptureReuse {
  ...
  prefix_each(List<unsigned int> prefix, const List<List<unsigned int>> &xss);
  ...
  static inline const unsigned int len_sum = []() {
    ...
    if (std::holds_alternative<typename List<List<unsigned int>>::Nil>(
      ...
      if (std::holds_alternative<typename List<List<unsigned int>>::Nil>(
        ...
        return (clone_as_value<List<unsigned int>>(d_a0).length() +
                clone_as_value<List<unsigned int>>(d_a00).length());
      }
    }
  }();
};
