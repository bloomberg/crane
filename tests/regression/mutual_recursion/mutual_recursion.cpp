#include "mutual_recursion.h"

bool MutualRecursion::even(unsigned int n) {
  if (n <= 0) {
    return true;
  } else {
    unsigned int n_ = n - 1;
    return odd(n_);
  }
}

bool MutualRecursion::odd(unsigned int n) {
  if (n <= 0) {
    return false;
  } else {
    unsigned int n_ = n - 1;
    return even(n_);
  }
}

unsigned int MutualRecursion::sum_even_indices(unsigned int n,
                                               unsigned int acc) {
  if (n <= 0) {
    return acc;
  } else {
    unsigned int n_ = n - 1;
    return sum_odd_indices(n_, acc);
  }
}

unsigned int MutualRecursion::sum_odd_indices(unsigned int n,
                                              unsigned int acc) {
  if (n <= 0) {
    return acc;
  } else {
    unsigned int n_ = n - 1;
    return sum_even_indices(n_, (acc + n));
  }
}

unsigned int MutualRecursion::process_a(unsigned int n, unsigned int m) {
  if (n <= 0) {
    return m;
  } else {
    unsigned int n_ = n - 1;
    return (process_b(n_, m) + n);
  }
}

unsigned int MutualRecursion::process_b(unsigned int n, unsigned int m) {
  if (n <= 0) {
    return m;
  } else {
    unsigned int n_ = n - 1;
    return (process_a(n_, m) + m);
  }
}

unsigned int MutualRecursion::eval_expr(const MutualRecursion::expr &e) {
  if (std::holds_alternative<typename MutualRecursion::expr::Val>(e.v())) {
    const auto &[d_a0] = std::get<typename MutualRecursion::expr::Val>(e.v());
    return d_a0;
  } else if (std::holds_alternative<typename MutualRecursion::expr::BinOp>(
                 e.v())) {
    const auto &[d_a0, d_a1, d_a2] =
        std::get<typename MutualRecursion::expr::BinOp>(e.v());
    if (d_a0 <= 0) {
      return (eval_expr(*d_a1) + eval_expr(*d_a2));
    } else {
      unsigned int _x = d_a0 - 1;
      return (eval_expr(*d_a1) * eval_expr(*d_a2));
    }
  } else {
    const auto &[d_a0, d_a1] =
        std::get<typename MutualRecursion::expr::UnOp>(e.v());
    if (d_a0 <= 0) {
      return eval_expr(*d_a1);
    } else {
      unsigned int _x = d_a0 - 1;
      return 0u;
    }
  }
}

unsigned int MutualRecursion::f1(unsigned int n) {
  if (n <= 0) {
    return 0u;
  } else {
    unsigned int n_ = n - 1;
    return (1u + f2(n_));
  }
}

unsigned int MutualRecursion::f2(unsigned int n) {
  if (n <= 0) {
    return 0u;
  } else {
    unsigned int n_ = n - 1;
    return (2u + f3(n_));
  }
}

unsigned int MutualRecursion::f3(unsigned int n) {
  if (n <= 0) {
    return 0u;
  } else {
    unsigned int n_ = n - 1;
    return (3u + f1(n_));
  }
}
