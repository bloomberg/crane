...
NameClashScopeLeak::rotate(const std::shared_ptr<List<unsigned int>> &l) {
  if (std::holds_alternative<typename List<unsigned int>::Nil>(l->v())) {
    return List<unsigned int>::nil();
  } else {
    const auto &[d_a0, d_a1] =
        std::get<typename List<unsigned int>::Cons>(l->v());
    return d_a1->app(List<unsigned int>::cons(d_a0, List<unsigned int>::nil()));
  }
}

/// Two consecutive matches on different lists in the same function.
__attribute__((pure)) unsigned int
NameClashScopeLeak::heads_sum(const std::shared_ptr<List<unsigned int>> &l1,
...
__attribute__((pure)) unsigned int NameClashScopeLeak::first_two_sum(
...
__attribute__((pure)) unsigned int NameClashScopeLeak::branch_let_clash(
    const std::shared_ptr<List<unsigned int>> &l) {
  if (std::holds_alternative<typename List<unsigned int>::Nil>(l->v())) {
    return 0u;
  } else {
...
__attribute__((pure)) unsigned int
NameClashScopeLeak::triple_head(const std::shared_ptr<List<unsigned int>> &l1,
                                const std::shared_ptr<List<unsigned int>> &l2,
...
__attribute__((pure)) unsigned int NameClashScopeLeak::pair_match(
    const std::pair<std::shared_ptr<List<unsigned int>>,
                    std::shared_ptr<List<unsigned int>>>
        p) {
  const std::shared_ptr<List<unsigned int>> &l1 = p.first;
  const std::shared_ptr<List<unsigned int>> &l2 = p.second;
  unsigned int h1 = [&]() {
    if (std::holds_alternative<typename List<unsigned int>::Nil>(l1->v())) {
      return 0u;
    } else {
