...
unsigned int NumeralEdge::take_nat(unsigned int n) { return (n + 1u); }

/// 10. Numeral in match
unsigned int NumeralEdge::classify(unsigned int n) {
  if (n <= 0) {
    return 0u;
  } else {
    unsigned int n0 = n - 1;
...
bool NumeralEdge::is_big(unsigned int n) { return 100u <= n; }
