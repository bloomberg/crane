...
unsigned int NumeralStress::count_from(unsigned int n, unsigned int target) {
  if (n <= 0) {
    return 0u;
  } else {
    unsigned int n_ = n - 1;
...
bool NumeralStress::check_range(unsigned int n) {
  return (10u <= n && n <= 100u);
}

/// 10. Mixed nat and Z in one function
int64_t NumeralStress::mixed_arith(unsigned int n) {
