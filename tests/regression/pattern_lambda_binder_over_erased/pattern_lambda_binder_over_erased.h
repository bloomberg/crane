#ifndef INCLUDED_PATTERN_LAMBDA_BINDER_OVER_ERASED
#define INCLUDED_PATTERN_LAMBDA_BINDER_OVER_ERASED
...
template <typename T> struct Phi {
  ...
  // CREATORS
  static Phi<T> phi0(List<std::pair<Nat, Exp0<T>>> es) {
    return {std::move(es)};
  }
};
