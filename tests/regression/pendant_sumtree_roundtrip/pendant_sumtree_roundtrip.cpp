...
PendantSumtreeRoundtripCase::digit_of_nat(unsigned int n) {
  return Fin::of_nat_lt(n, 10u);
}

unsigned int PendantSumtreeRoundtripCase::value_digits(unsigned int,
                                                       const T0<T> &ds) {
  if (std::holds_alternative<typename T0<T>::Nil>(ds.v())) {
    return 0u;
  } else {
    const auto &[h, n, a2] = std::get<typename T0<T>::Cons>(ds.v());
    return (digit_to_nat(h) + (10u * value_digits(n, *a2)));
  }
}

std::optional<T0<PendantSumtreeRoundtripCase::digit>>
PendantSumtreeRoundtripCase::list_to_vector_opt(unsigned int n,
                                                const List<T> &xs) {
  if (n <= 0) {
...
PendantSumtreeRoundtripCase::sumtree_top(
    unsigned int, const PendantSumtreeRoundtripCase::SumTree &st) {
...
bool PendantSumtreeRoundtripCase::sumtree_validb_aux(
    unsigned int n, unsigned int fuel,
...
bool PendantSumtreeRoundtripCase::nat_list_eqb(const List<unsigned int> &xs,
  ...
  if (std::holds_alternative<typename List<unsigned int>::Nil0>(xs.v())) {
    ...
    const auto &[a0, a1] = std::get<typename List<unsigned int>::Cons0>(xs.v());
    if (std::holds_alternative<typename List<unsigned int>::Nil0>(ys.v())) {
      return false;
    } else {
      const auto &[a00, a10] =
...
T Fin::of_nat_lt(unsigned int p, unsigned int n) {
  if (n <= 0) {
