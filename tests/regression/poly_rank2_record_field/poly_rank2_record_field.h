#ifndef INCLUDED_POLY_RANK2_RECORD_FIELD
...
public:
  ...
  Bool0 eqb(const Nat &m0) const {
    const Nat *_loop_self = this;
    const Nat *_loop_m0 = &m0;
    while (true) {
      auto &&_sv = *_loop_self;
      if (std::holds_alternative<typename Nat::O>(_sv.v())) {
        if (std::holds_alternative<typename Nat::O>(_loop_m0->v())) {
          return Bool0::TRUE_;
        } else {
          return Bool0::FALSE_;
        }
        ...
        if (std::holds_alternative<typename Nat::O>(_loop_m0->v())) {
          ...
          const auto &[a00] = std::get<typename Nat::S>(_loop_m0->v());
          _loop_self = crane_raw(a0);
          _loop_m0 = crane_raw(a00);
        }
      }
    }
  }
...
struct PolyRank2RecordField {
  ...
  static List<T2> run(const mapper &m0, F1 &&x, const List<T1> &x0) {
  ...
  static List<Nat> test2(const List<Bool0> &l);
};

#endif // INCLUDED_POLY_RANK2_RECORD_FIELD
