#ifndef INCLUDED_PROD_FST_PROJECTION
#define INCLUDED_PROD_FST_PROJECTION
...
struct ProdFstProjection {
  struct t {
    // TYPES
    struct L {};

    struct N {
      std::shared_ptr<std::pair<t, uint64_t>> a0;
    };
  ...
  public:
    ...
    static t n(std::pair<t, uint64_t> a0) {
      return t(N{std::make_shared<std::pair<t, uint64_t>>(std::move(a0))});
    }
  ...
  template <typename T1, typename F1>
    requires std::is_invocable_r_v<T1, F1 &, std::pair<t, uint64_t> &>
...
#endif // INCLUDED_PROD_FST_PROJECTION
