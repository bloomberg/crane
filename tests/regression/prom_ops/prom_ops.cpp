...
unsigned int PromOps::prom_data_or_zero(const PromOps::state1 &s) {
  if (s.prom_enable1) {
    return s.prom_data1;
  } else {
    return 0u;
...
unsigned int PromOps::flagged_sum(const PromOps::state2 &s) {
...
PromOps::state3 PromOps::set_prom_params3(const PromOps::state3 &s,
                                          ...
                                          const bool enable) {
  return state3(s.acc3, s.regs3, s.carry3, s.pc3, s.stack3, s.ram_sys3,
...
PromOps::state10 PromOps::set_prom_params10(const PromOps::state10 &s,
                                            ...
                                            const unsigned int data,
                                            const bool enable) {
  return state10(s.regs10, s.rom10, s.acc10, s.pc10, s.stack10, s.cur_bank10,
...
PromOps::state10 PromOps::execute_wpm10(const PromOps::state10 &s) {
  ...
  return state10(s.regs10, new_rom, s.acc10, s.pc10, s.stack10, s.cur_bank10,
