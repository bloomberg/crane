#ifndef INCLUDED_PROMOTED_VAR_COLLAPSE
#define INCLUDED_PROMOTED_VAR_COLLAPSE
...
concept Prov = requires {
  ...
  typename I::prov;
  {
    I::prov_size(std::declval<typename I::prov>())
...
struct PromotedVarCollapse {
  template <Params _tcI0>
  static Nat takes_three(const typename _tcI0::PROV::allocationId &,
                         const typename _tcI0::PROV::prov &,
                         const List<typename _tcI0::PROV::prov> &,
                         const Nat &n) {
    return n.add(_tcI0::width());
  }
};

#endif // INCLUDED_PROMOTED_VAR_COLLAPSE
