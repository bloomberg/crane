#include <ram_accessor.h>

#include <memory>
#include <type_traits>
#include <utility>
#include <variant>

__attribute__((pure)) unsigned int
RamAccessor::get_main(const std::shared_ptr<RamAccessor::ram_reg> &rg,
                      const unsigned int i) {
  return ListDef::template nth<unsigned int>(i, rg->reg_main, 0u);
}

__attribute__((pure)) unsigned int
RamAccessor::get_stat(const std::shared_ptr<RamAccessor::ram_reg> &rg,
                      const unsigned int i) {
  return ListDef::template nth<unsigned int>(i, rg->reg_status, 0u);
}

std::shared_ptr<RamAccessor::ram_reg>
RamAccessor::upd_main_in_reg(const std::shared_ptr<RamAccessor::ram_reg> &rg,
                             const unsigned int i, const unsigned int v) {
  return std::make_shared<RamAccessor::ram_reg>(
      ram_reg{update_nth<unsigned int>(i, (16u ? v % 16u : v), rg->reg_main),
              rg->reg_status});
}

std::shared_ptr<RamAccessor::ram_reg>
RamAccessor::upd_stat_in_reg(const std::shared_ptr<RamAccessor::ram_reg> &rg,
                             const unsigned int i, const unsigned int v) {
  return std::make_shared<RamAccessor::ram_reg>(
      ram_reg{rg->reg_main, update_nth<unsigned int>(i, (16u ? v % 16u : v),
                                                     rg->reg_status)});
}

std::shared_ptr<RamAccessor::ram_reg>
RamAccessor::get_regRAM(const std::shared_ptr<RamAccessor::ram_chip> &ch,
                        const unsigned int r) {
  return ListDef::template nth<std::shared_ptr<RamAccessor::ram_reg>>(
      r, ch->chip_regs, empty_reg);
}

std::shared_ptr<RamAccessor::ram_chip>
RamAccessor::upd_reg_in_chip(const std::shared_ptr<RamAccessor::ram_chip> &ch,
                             const unsigned int r,
                             const std::shared_ptr<RamAccessor::ram_reg> &rg) {
  return std::make_shared<RamAccessor::ram_chip>(ram_chip{
      update_nth<std::shared_ptr<RamAccessor::ram_reg>>(r, rg, ch->chip_regs),
      ch->chip_port});
}

std::shared_ptr<RamAccessor::ram_chip>
RamAccessor::upd_port_in_chip(const std::shared_ptr<RamAccessor::ram_chip> &ch,
                              const unsigned int v) {
  return std::make_shared<RamAccessor::ram_chip>(
      ram_chip{ch->chip_regs, (16u ? v % 16u : v)});
}

std::shared_ptr<RamAccessor::ram_chip>
RamAccessor::get_chip(const std::shared_ptr<RamAccessor::ram_bank> &bk,
                      const unsigned int c) {
  return ListDef::template nth<std::shared_ptr<RamAccessor::ram_chip>>(
      c, bk->bank_chips, empty_chip);
}

std::shared_ptr<RamAccessor::ram_bank> RamAccessor::upd_chip_in_bank(
    const std::shared_ptr<RamAccessor::ram_bank> &bk, const unsigned int c,
    const std::shared_ptr<RamAccessor::ram_chip> &ch) {
  return std::make_shared<RamAccessor::ram_bank>(
      ram_bank{update_nth<std::shared_ptr<RamAccessor::ram_chip>>(
          c, ch, bk->bank_chips)});
}

std::shared_ptr<RamAccessor::ram_bank> RamAccessor::get_bank_from_sys(
    const std::shared_ptr<List<std::shared_ptr<RamAccessor::ram_bank>>> &sys,
    const unsigned int b) {
  return ListDef::template nth<std::shared_ptr<RamAccessor::ram_bank>>(
      b, sys, empty_bank);
}

std::shared_ptr<List<std::shared_ptr<RamAccessor::ram_bank>>>
RamAccessor::upd_bank_in_sys(const std::shared_ptr<RamAccessor::state> &s,
                             const unsigned int b,
                             const std::shared_ptr<RamAccessor::ram_bank> &bk) {
  return update_nth<std::shared_ptr<RamAccessor::ram_bank>>(b, bk,
                                                            s->state_ram);
}

std::shared_ptr<RamAccessor::ram_bank>
RamAccessor::current_bank(const std::shared_ptr<RamAccessor::state> &s) {
  return get_bank_from_sys(s->state_ram, s->state_sel->sel_bank);
}

std::shared_ptr<RamAccessor::ram_chip>
RamAccessor::current_chip(const std::shared_ptr<RamAccessor::state> &s) {
  return get_chip(current_bank(s), s->state_sel->sel_chip);
}

std::shared_ptr<RamAccessor::ram_reg>
RamAccessor::current_reg(const std::shared_ptr<RamAccessor::state> &s) {
  return get_regRAM(current_chip(s), s->state_sel->sel_reg);
}

__attribute__((pure)) unsigned int
RamAccessor::ram_read_main(const std::shared_ptr<RamAccessor::state> &s) {
  return get_main(current_reg(s), s->state_sel->sel_char);
}

std::shared_ptr<List<std::shared_ptr<RamAccessor::ram_bank>>>
RamAccessor::ram_write_main_sys(const std::shared_ptr<RamAccessor::state> &s,
                                const unsigned int v) {
  std::shared_ptr<RamAccessor::ram_reg> rg = current_reg(s);
  std::shared_ptr<RamAccessor::ram_chip> ch = current_chip(s);
  std::shared_ptr<RamAccessor::ram_bank> bk = current_bank(s);
  std::shared_ptr<RamAccessor::ram_reg> rg_ =
      upd_main_in_reg(std::move(rg), s->state_sel->sel_char, v);
  std::shared_ptr<RamAccessor::ram_chip> ch_ =
      upd_reg_in_chip(std::move(ch), s->state_sel->sel_reg, std::move(rg_));
  std::shared_ptr<RamAccessor::ram_bank> bk_ =
      upd_chip_in_bank(std::move(bk), s->state_sel->sel_chip, std::move(ch_));
  return upd_bank_in_sys(s, s->state_sel->sel_bank, std::move(bk_));
}

std::shared_ptr<List<std::shared_ptr<RamAccessor::ram_bank>>>
RamAccessor::ram_write_status_sys(const std::shared_ptr<RamAccessor::state> &s,
                                  const unsigned int idx,
                                  const unsigned int v) {
  std::shared_ptr<RamAccessor::ram_reg> rg = current_reg(s);
  std::shared_ptr<RamAccessor::ram_chip> ch = current_chip(s);
  std::shared_ptr<RamAccessor::ram_bank> bk = current_bank(s);
  std::shared_ptr<RamAccessor::ram_reg> rg_ =
      upd_stat_in_reg(std::move(rg), idx, v);
  std::shared_ptr<RamAccessor::ram_chip> ch_ =
      upd_reg_in_chip(std::move(ch), s->state_sel->sel_reg, std::move(rg_));
  std::shared_ptr<RamAccessor::ram_bank> bk_ =
      upd_chip_in_bank(std::move(bk), s->state_sel->sel_chip, std::move(ch_));
  return upd_bank_in_sys(s, s->state_sel->sel_bank, std::move(bk_));
}
