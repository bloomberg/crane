...
(* Behavioral candidate: nested RAM main-character write update chain. *)

From Stdlib Require Import List Nat.
Import ListNotations.

Module RamMainWriteChain.
...
Definition upd_main_in_reg (rg : ram_reg) (i : nat) (v : nat) : ram_reg :=
  mkRamReg (update_nth i (v mod 16) (reg_main rg)).
...
Definition t : nat :=
  let rg0 := mkRamReg [0; 0; 0] in
  let ch0 := mkRamChip [rg0] in
  let bk0 := mkRamBank [ch0] in
  let s := mkState [bk0] 0 0 0 1 in
...
End RamMainWriteChain.

Require Crane.Extraction.
From Crane Require Mapping.Std Mapping.NatIntStd.
Crane Extraction "ram_main_write_chain" RamMainWriteChain.
