...
RegisterPairOps::get_reg(const RegisterPairOps::state &s,
                         const unsigned int &r) {
  return ListDef::template nth<unsigned int>(r, s.regs, 0u);
}

__attribute__((pure)) RegisterPairOps::state
RegisterPairOps::set_reg(const RegisterPairOps::state &s, const unsigned int &r,
                         const unsigned int &v) {
  return state{update_nth<unsigned int>(r, (16u ? v % 16u : v), s.regs)};
}

__attribute__((pure)) unsigned int
RegisterPairOps::get_reg_pair(const RegisterPairOps::state &s,
                              const unsigned int &r) {
  unsigned int base =
