...
struct RocqBug10757 {
  ...
  iterate_func(F0 &&beq, F1 &&f,
    ...
    std::function<std::shared_ptr<Sig<T1>>(T1)> iterate0 =
        [=](const T1 x1) mutable {
          std::shared_ptr<Sig<T1>> y =
              Sig<std::shared_ptr<Sig<T1>>>::exist(Sig<T1>::exist(x1));
  ...
  template <typename T1, MapsTo<Bool0, T1, T1> F0, MapsTo<T1, T1> F1>
  static std::shared_ptr<Sig<T1>> iterate(F0 &&beq, F1 &&f, const T1 x) {
