...
struct SrcWrrRomPortRoundtrip {
  ...
  struct state {
    ...
    __attribute__((pure)) state clone() const {
      return state{clone_as_value<List<unsigned int>>((*(this)).regs),
                   clone_as_value<unsigned int>((*(this)).acc),
                   clone_as_value<List<unsigned int>>((*(this)).rom_ports),
                   clone_as_value<unsigned int>((*(this)).sel_rom)};
    }
  };
  ...
  __attribute__((pure)) static state execute_src(const state &s,
                                                 const unsigned int &r);
  __attribute__((pure)) static state execute_wrr(const state &s);
  static inline const state sample = state{
      ...
      0u};
  static inline const state after_src = execute_src(sample, 3u);
  static inline const state after_wrr = execute_wrr(after_src);
  static inline const bool t =
      ListDef::template nth<unsigned int>(after_src.sel_rom,
                                          after_wrr.rom_ports, 0u) == 13u;
};
