...
struct UnitVoidEdge2 {
  ...
  __attribute__((pure)) static unsigned int
  let_match_unit(const unsigned int n);
  __attribute__((pure)) static unsigned int let_chain_use(const unsigned int n);
  ...
  __attribute__((pure)) static unsigned int
  call_and_discard(F0 &&, const unsigned int n) {
    return n;
  }

  template <MapsTo<void, unsigned int> F0>
  __attribute__((pure)) static unsigned int call_and_use(F0 &&f,
  ...
  static inline const unsigned int use_pair = []() {
    std::shared_ptr<pair<unsigned int, std::monostate>> p =
        make_nat_unit_pair(7u);
