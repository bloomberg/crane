...
WpmOps::state1 WpmOps::execute_wpm1(const WpmOps::state1 &s) {
  ...
  return state1{std::move(new_rom), s.prom_addr1, s.prom_data1, s.prom_enable1};
}
...
WpmOps::state2 WpmOps::execute_wpm2(const WpmOps::state2 &s) {
  ...
  return state2{s.ram_sys2, std::move(new_rom), s.prom_addr2, s.prom_data2,
                s.prom_enable2};
}
...
WpmOps::state3 WpmOps::execute_wpm3(const WpmOps::state3 &s) {
  ...
  return state3{s.regs3, std::move(new_rom), s.prom_addr3, s.prom_data3,
                s.prom_enable3};
}
...
WpmOps::state4 WpmOps::execute_wpm4(const WpmOps::state4 &s) {
  ...
  return state4{std::move(new_rom), s.prom_addr4, s.prom_data4, s.prom_enable4};
}
...
WpmOps::state5 WpmOps::execute_wpm5(const WpmOps::state5 &s) {
  ...
  return state5{std::move(new_rom), s.prom_addr5, s.prom_data5, s.prom_enable5};
}
...
WpmOps::state6 WpmOps::execute_wpm6(const WpmOps::state6 &s) {
  ...
  return state6{std::move(new_rom), s.prom_addr6, s.prom_data6, s.prom_enable6};
}
