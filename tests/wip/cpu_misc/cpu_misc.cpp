...
unsigned int CpuMisc::get_reg(const std::shared_ptr<CpuMisc::state> &s,
                              const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
CpuMisc::set_reg(const std::shared_ptr<CpuMisc::state> &s, const unsigned int r,
                 const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int CpuMisc::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int CpuMisc::get_pair(const std::shared_ptr<CpuMisc::state> &s,
                               const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
CpuMisc::set_pair(const std::shared_ptr<CpuMisc::state> &s,
...
std::shared_ptr<List<unsigned int>>
CpuMisc::push_return(std::shared_ptr<CpuMisc::state> s,
...
std::shared_ptr<CpuMisc::state>
CpuMisc::execute(std::shared_ptr<CpuMisc::state> s,
                 const std::shared_ptr<CpuMisc::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename CpuMisc::instr::NOP _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename CpuMisc::instr::LDM _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int n = _args._a0;
            return std::make_shared<CpuMisc::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename CpuMisc::instr::LD _args)
              -> std::shared_ptr<CpuMisc::state> {
          ...
          [&](const typename CpuMisc::instr::XCH _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename CpuMisc::instr::INC _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            return std::make_shared<CpuMisc::state>(
          ...
          [&](const typename CpuMisc::instr::ADD _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename CpuMisc::instr::SUB _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename CpuMisc::instr::IAC _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
          ...
          [&](const typename CpuMisc::instr::DAC _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
          ...
          [&](const typename CpuMisc::instr::CLC _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename CpuMisc::instr::STC _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename CpuMisc::instr::CMC _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
          ...
          [&](const typename CpuMisc::instr::CMA _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
          ...
          [&](const typename CpuMisc::instr::CLB _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename CpuMisc::instr::RAL _args)
              -> std::shared_ptr<CpuMisc::state> {
          ...
          [&](const typename CpuMisc::instr::RAR _args)
              -> std::shared_ptr<CpuMisc::state> {
          ...
          [&](const typename CpuMisc::instr::TCC _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename CpuMisc::instr::TCS _args)
              -> std::shared_ptr<CpuMisc::state> {
            return std::make_shared<CpuMisc::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename CpuMisc::instr::DAA _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename CpuMisc::instr::KBP _args)
              -> std::shared_ptr<CpuMisc::state> {
          ...
          [&](const typename CpuMisc::instr::JUN _args)
              -> std::shared_ptr<CpuMisc::state> {
          ...
          [&](const typename CpuMisc::instr::JMS _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int a = _args._a0;
            return std::make_shared<CpuMisc::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename CpuMisc::instr::JCN _args)
              -> std::shared_ptr<CpuMisc::state> {
          ...
          [&](const typename CpuMisc::instr::FIM _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename CpuMisc::instr::SRC _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            return std::make_shared<CpuMisc::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename CpuMisc::instr::FIN _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            return std::make_shared<CpuMisc::state>(
          ...
          [&](const typename CpuMisc::instr::JIN _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            return std::make_shared<CpuMisc::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename CpuMisc::instr::ISZ _args)
              -> std::shared_ptr<CpuMisc::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename CpuMisc::instr::BBL _args)
              -> std::shared_ptr<CpuMisc::state> {
