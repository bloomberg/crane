...
#include <any>
#include <cassert>
#include <decode_opcode_6_to_13_wf_edge_0049.h>
#include <functional>
#include <iostream>
#include <memory>
#include <optional>
...
DecodeOpcode6To13WfEdge0049::update_nth(const unsigned int n,
...
std::shared_ptr<DecodeOpcode6To13WfEdge0049::state>
DecodeOpcode6To13WfEdge0049::set_prom_params(
    std::shared_ptr<DecodeOpcode6To13WfEdge0049::state> s,
    const unsigned int addr, const unsigned int data, const bool enable) {
  return std::make_shared<DecodeOpcode6To13WfEdge0049::state>(state{
      std::move(s)->rom, std::move(addr), std::move(data), std::move(enable)});
}

std::shared_ptr<DecodeOpcode6To13WfEdge0049::state>
DecodeOpcode6To13WfEdge0049::execute_wpm(
    std::shared_ptr<DecodeOpcode6To13WfEdge0049::state> s) {
  ...
  return std::make_shared<DecodeOpcode6To13WfEdge0049::state>(
      state{std::move(new_rom), s->prom_addr, s->prom_data, s->prom_enable});
}

std::shared_ptr<DecodeOpcode6To13WfEdge0049::state>
DecodeOpcode6To13WfEdge0049::load_program(
    std::shared_ptr<DecodeOpcode6To13WfEdge0049::state> s,
    const unsigned int base, const std::shared_ptr<List<unsigned int>> &bytes) {
  return std::visit(
      Overloaded{[&](const typename List<unsigned int>::nil _args)
                     -> std::shared_ptr<DecodeOpcode6To13WfEdge0049::state> {
                   return std::move(s);
                 ...
                 [&](const typename List<unsigned int>::cons _args)
                   ...
                   std::shared_ptr<DecodeOpcode6To13WfEdge0049::state> s1 =
                       set_prom_params(std::move(s), base, std::move(b), true);
                   std::shared_ptr<DecodeOpcode6To13WfEdge0049::state> s2 =
