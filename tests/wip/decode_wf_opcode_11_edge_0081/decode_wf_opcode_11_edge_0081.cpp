...
#include <any>
#include <cassert>
#include <decode_wf_opcode_11_edge_0081.h>
#include <functional>
#include <iostream>
#include <memory>
#include <optional>
...
DecodeWfOpcode11Edge0081::update_nth(const unsigned int n, const unsigned int x,
...
std::shared_ptr<DecodeWfOpcode11Edge0081::state>
DecodeWfOpcode11Edge0081::set_prom_params(
    std::shared_ptr<DecodeWfOpcode11Edge0081::state> s, const unsigned int addr,
    const unsigned int data, const bool enable) {
  return std::make_shared<DecodeWfOpcode11Edge0081::state>(state{
      std::move(s)->rom, std::move(addr), std::move(data), std::move(enable)});
}

std::shared_ptr<DecodeWfOpcode11Edge0081::state>
DecodeWfOpcode11Edge0081::execute_wpm(
    std::shared_ptr<DecodeWfOpcode11Edge0081::state> s) {
  ...
  return std::make_shared<DecodeWfOpcode11Edge0081::state>(
      state{std::move(new_rom), s->prom_addr, s->prom_data, s->prom_enable});
}

std::shared_ptr<DecodeWfOpcode11Edge0081::state>
DecodeWfOpcode11Edge0081::load_program(
    std::shared_ptr<DecodeWfOpcode11Edge0081::state> s, const unsigned int base,
  ...
  return std::visit(
      Overloaded{[&](const typename List<unsigned int>::nil _args)
                 ...
                 [&](const typename List<unsigned int>::cons _args)
                   ...
                   std::shared_ptr<DecodeWfOpcode11Edge0081::state> s1 =
                       set_prom_params(std::move(s), base, std::move(b), true);
                   std::shared_ptr<DecodeWfOpcode11Edge0081::state> s2 =
