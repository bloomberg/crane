...
struct DisassembleDropWindow {
  ...
  public:
    ...
    using variant_t = std::variant<NOP_, LDM_>;

  private:
    variant_t v_;
    explicit instruction(NOP_ _v) : v_(std::move(_v)) {}
    explicit instruction(LDM_ _v) : v_(std::move(_v)) {}
  ...
  public:
    struct ctor {
      ...
      static std::shared_ptr<instruction> LDM__(unsigned int a0) {
        return std::shared_ptr<instruction>(new instruction(LDM_{a0}));
      }
      static std::unique_ptr<instruction> NOP__uptr() {
        return std::unique_ptr<instruction>(new instruction(NOP_{}));
      }
      static std::unique_ptr<instruction> LDM__uptr(unsigned int a0) {
        return std::unique_ptr<instruction>(new instruction(LDM_{a0}));
      }
    };
    const variant_t &v() const { return v_; }
    variant_t &v_mut() { return v_; }
  ...
  static T1 instruction_rect(const T1 f, F1 &&f0,
    ...
    return std::visit(
        Overloaded{
            ...
            [&](const typename instruction::LDM_ _args) -> T1 {
