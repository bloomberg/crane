...
struct FixMoveCapture {
  ...
  static T1 mylist_rect(const T1 f0, F1 &&f1,
                        const std::shared_ptr<mylist> &m) {
  ...
  template <typename T1,
            MapsTo<T1, unsigned int, std::shared_ptr<mylist>, T1> F1>
  static T1 mylist_rec(const T1 f0, F1 &&f1, const std::shared_ptr<mylist> &m) {
    if (std::holds_alternative<typename mylist::Mynil>(m->v())) {
      return f0;
    } else {
      const auto &[d_a0, d_a1] = std::get<typename mylist::Mycons>(m->v());
  ...
  sum(const std::shared_ptr<mylist>
          &l); /// dup_head stores l in the constructor → l escapes → owned.
  /// This means the caller passes l by value (move semantics).
  static std::shared_ptr<mylist> dup_head(std::shared_ptr<mylist> l);
  ...
  __attribute__((pure)) static unsigned int f(std::shared_ptr<mylist> l);
  ...
  __attribute__((pure)) static unsigned int f2(std::shared_ptr<mylist> l);
