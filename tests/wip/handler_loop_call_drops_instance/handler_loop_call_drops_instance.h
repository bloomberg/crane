#ifndef INCLUDED_HANDLER_LOOP_CALL_DROPS_INSTANCE
#define INCLUDED_HANDLER_LOOP_CALL_DROPS_INSTANCE
...
struct Denot {
  ...
  static std::optional<exc<typename _tcI0::ptr>>
  exc_of_event(const Sum1<OtherE, FailE<typename _tcI0::ptr>, std::any> &e) {
    if (std::holds_alternative<
            typename Sum1<OtherE, FailE<typename _tcI0::ptr>, std::any>::Inl1>(
            e.v())) {
      return std::optional<dvalue<typename _tcI0::ptr>>();
    } else {
      const auto &[a0] = std::get<
          typename Sum1<OtherE, FailE<typename _tcI0::ptr>, std::any>::Inr1>(
          e.v());
      const auto &[a00] = a0;
      return std::make_optional<dvalue<typename _tcI0::ptr>>(a00);
    }
  }
...
struct HandlerLoopCallDropsInstance {
  static std::shared_ptr<
      ITree<Sum<Denot::template exc<typename natParams::ptr>, Nat>>>
  run();
};

#endif // INCLUDED_HANDLER_LOOP_CALL_DROPS_INSTANCE
