...
template <Monad _tcI0> struct Functor_Monad {
  ...
  static T1<std::any, std::any> fmap(std::function<std::any(std::any)> a0,
                                     T1<std::any, std::any> a1) {
    return Monad0::liftM(_db5, std::move(a0), std::move(a1));
  }
};
...
struct Monad_option {
  ...
  template <typename _A0> static std::optional<_A0> ret(_A0 _x) {
    return std::make_optional<_A0>(_x);
  }
