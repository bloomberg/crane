#ifndef INCLUDED_ITREE_BIND_BINDER_AT_NAMED_DECL
#define INCLUDED_ITREE_BIND_BINDER_AT_NAMED_DECL
...
return {a0};}
  // CREATORS
static FailE Throw_(std::monostate a0) {
return {a0};}
};template <typename
...
};template <Params _tcI0>std::shared_ptr<ITree<Sum<Nat,
Dval<typename _tcI0::PTR::ptr,
typename _tcI0::IPTR::iptr>>>> runS(){return itree_ret(Sum<Nat,
Dval<typename _tcI0::PTR::ptr,
...
return Nat::o();}
};struct ItreeBindBinderAtNamedDecl {
static std::shared_ptr<ITree<Sum<Nat,
Dval<typename ParamsV<natIPtr>::PTR::ptr,
typename ParamsV<natIPtr>::IPTR::iptr>>>> runM();
static Nat to_natM(const Dval<typename ParamsV<natIPtr>::PTR::ptr,
typename ParamsV<natIPtr>::IPTR::iptr>& d);
static std::shared_ptr<ITree<Nat>> check();
};

#endif // INCLUDED_ITREE_BIND_BINDER_AT_NAMED_DECL
