...
#include <functional>
#include <iostream>
#include <jcn_branch_target_choice.h>
#include <memory>
#include <optional>
#include <stdexcept>
#include <string>
...
unsigned int JcnBranchTargetChoice::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int JcnBranchTargetChoice::pc_inc2(
    const std::shared_ptr<JcnBranchTargetChoice::state> &s) {
  return addr12_of_nat((s->pc + 2u));
}

unsigned int JcnBranchTargetChoice::page_of(const unsigned int p) {
  return Nat::div(p, 256u);
}

unsigned int JcnBranchTargetChoice::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int JcnBranchTargetChoice::base_for_next2(
    const std::shared_ptr<JcnBranchTargetChoice::state> &s) {
  return page_base(pc_inc2(s));
}

bool JcnBranchTargetChoice::jcn_condition(
    const std::shared_ptr<JcnBranchTargetChoice::state> &s,
...
unsigned int JcnBranchTargetChoice::branch_target(
    const std::shared_ptr<JcnBranchTargetChoice::state> &s,
