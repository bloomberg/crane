...
uint64_t _anon_outer(const T1, const LiftedLambdaNested::t x) {
  std::function<uint64_t(T1)> inner = [=](const T1 &) mutable {
    return LiftedLambdaNested::depth(x);
  };
  return (inner(UINT64_C(0)) + LiftedLambdaNested::depth(x));
}
