...
struct ListSelfDeepCopy {
  ...
  /// own iterative clone, but cloning each element can still re-enter chain
  /// recursively for a single-child chain.
  struct chain {
    // TYPES
    struct Stop {};
  ...
  public:
    ...
    chain clone() const {
      ...
      if (std::holds_alternative<Stop>(_sv.v())) {
        ...
        return chain(Link{
            d_a0
                ? std::make_unique<List<ListSelfDeepCopy::chain>>(d_a0->clone())
                : nullptr});
      }
    }
  ...
  template <typename T1, MapsTo<T1, List<chain>> F1>
