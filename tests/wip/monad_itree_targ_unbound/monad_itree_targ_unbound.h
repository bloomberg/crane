...
Monads::template stateT<env, std::shared_ptr, Nat> step(Nat n) {
...
Monads::template stateT<env, std::shared_ptr, T2> handle(Ev) {
  return [](Nat s) {
    if (s.eqb(Nat::o())) {
...
Monads::template stateT<env, std::shared_ptr, T2> twice(Ev e) {
