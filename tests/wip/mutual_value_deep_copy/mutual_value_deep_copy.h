...
struct MutualValueDeepCopy {
  ...
  /// A deep alternating value can still force recursive copy construction if
  /// the mutual case is not handled iteratively.
  struct a;
  struct b;
  ...
  public:
    ...
    a clone() const {
      auto &&_sv = *(this);
      if (std::holds_alternative<AEnd>(_sv.v())) {
        return a(AEnd{});
      } else {
        const auto &[d_a0, d_a1] = std::get<ANode>(_sv.v());
        return a(ANode{
            d_a0, d_a1 ? std::make_unique<MutualValueDeepCopy::b>(d_a1->clone())
                       : nullptr});
      }
    }
  ...
  template <typename T1, MapsTo<T1, bool, b> F1>
