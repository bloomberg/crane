...
struct PairSelfDeepCopy {
  ...
  /// std::pair containing an owned recursive value, and clone generation must
  /// understand that container shape.
  struct chain {
    // TYPES
    struct Stop {};
  ...
  template <typename T1, MapsTo<T1, std::pair<chain, bool>> F1>
