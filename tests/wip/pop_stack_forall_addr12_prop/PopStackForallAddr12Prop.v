...
(* Behavioral candidate: pop_stack preserves 12-bit address bounds on the remaining stack. *)

From Stdlib Require Import List Nat Bool.
Import ListNotations.

Module PopStackForallAddr12Prop.
...
Definition t : Prop :=
  Forall (fun a => a < 4096) (state_stack stack_state) /\
  match pop_stack stack_state with
  | (Some a, s') => a < 4096 /\ Forall (fun x => x < 4096) (state_stack s')
  | _ => False
  end.

End PopStackForallAddr12Prop.

Require Crane.Extraction.
From Crane Require Mapping.Std Mapping.NatIntStd.
Crane Extraction "pop_stack_forall_addr12_prop" PopStackForallAddr12Prop.
