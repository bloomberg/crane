...
#include <memory>
#include <optional>
#include <ram_write_status_sys_edge_0017.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>
...
std::shared_ptr<RamWriteStatusSysEdge0017::state>
RamWriteStatusSysEdge0017::set_prom_params(
    std::shared_ptr<RamWriteStatusSysEdge0017::state> s,
    const unsigned int addr, const unsigned int data, const bool enable) {
  return std::make_shared<RamWriteStatusSysEdge0017::state>(state{
      std::move(s)->rom, std::move(addr), std::move(data), std::move(enable)});
}

std::shared_ptr<RamWriteStatusSysEdge0017::state>
RamWriteStatusSysEdge0017::execute_wpm(
    std::shared_ptr<RamWriteStatusSysEdge0017::state> s) {
  ...
  return std::make_shared<RamWriteStatusSysEdge0017::state>(
      state{std::move(new_rom), s->prom_addr, s->prom_data, s->prom_enable});
}

std::shared_ptr<RamWriteStatusSysEdge0017::state>
RamWriteStatusSysEdge0017::load_program(
    std::shared_ptr<RamWriteStatusSysEdge0017::state> s,
  ...
  return std::visit(
      Overloaded{[&](const typename List<unsigned int>::nil _args)
                 ...
                 [&](const typename List<unsigned int>::cons _args)
                   ...
                   std::shared_ptr<RamWriteStatusSysEdge0017::state> s1 =
                       set_prom_params(std::move(s), base, std::move(b), true);
                   std::shared_ptr<RamWriteStatusSysEdge0017::state> s2 =
