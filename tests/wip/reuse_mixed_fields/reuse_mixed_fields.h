...
struct ReuseMixedFields {
  ...
  public:
    ...
    static std::shared_ptr<payload> asnat(unsigned int a0, unsigned int a1) {
      return std::make_shared<payload>(AsNat{std::move(a0), std::move(a1)});
    }

    static std::shared_ptr<payload> aspair(unsigned int a0, unsigned int a1) {
      return std::make_shared<payload>(AsPair{std::move(a0), std::move(a1)});
    }
  ...
  static T1 payload_rect(F0 &&f, F1 &&f0, const std::shared_ptr<payload> &p) {
  ...
  template <typename T1, MapsTo<T1, unsigned int, unsigned int> F0,
            MapsTo<T1, unsigned int, unsigned int> F1>
  static T1 payload_rec(F0 &&f, F1 &&f0, const std::shared_ptr<payload> &p) {
    if (std::holds_alternative<typename payload::AsNat>(p->v())) {
      const auto &[d_a0, d_a1] = std::get<typename payload::AsNat>(p->v());
      return f(d_a0, d_a1);
    } else {
  ...
  /// have arity 2.
  static std::shared_ptr<payload> swap_tag_or_id(std::shared_ptr<payload> p,
                                                 const bool do_swap);
  ...
  static inline const unsigned int test1 = []() {
    ...
    if (std::holds_alternative<typename payload::AsNat>(_sv0->v())) {
      ...
      const auto &[d_a00, d_a10] =
          std::get<typename payload::AsPair>(_sv0->v());
      return d_a00;
    }
  }();
  ...
  static inline const unsigned int test2 = []() {
    ...
    if (std::holds_alternative<typename payload::AsNat>(_sv1->v())) {
      ...
      const auto &[d_a01, d_a11] =
          std::get<typename payload::AsPair>(_sv1->v());
