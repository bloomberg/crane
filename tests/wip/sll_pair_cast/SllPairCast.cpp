...
bool SllPairCast::sll_final_config(const SllPairCast::sll_subparser &sp) {
  ...
  std::pair<SllPairCast::sll_frame, Datatypes::List<SllPairCast::sll_frame>>
      sll_stk0 = sp.sll_stk;
  const auto &[s, l] = std::any_cast<std::pair<std::any, std::any>>(sll_stk0);
  std::optional<uint64_t> fr_ret0 =
      std::any_cast<SllPairCast::sll_frame>(
          std::any_cast<SllPairCast::sll_frame>(s))
          .fr_ret;
  ...
  if (fr_ret0.has_value()) {
    ...
    if (std::holds_alternative<typename Datatypes::List<uint64_t>::Nil>(
            fr_suf0.v())) {
      auto &&_sv = Datatypes::List<SllPairCast::sll_frame>(
          std::any_cast<Datatypes::List<std::any>>(
              Datatypes::List<SllPairCast::sll_frame>(
                  std::any_cast<Datatypes::List<std::any>>(l))));
      if (std::holds_alternative<
              typename Datatypes::List<SllPairCast::sll_frame>::Nil>(_sv.v())) {
        return true;
      } else {
        return false;
      }
