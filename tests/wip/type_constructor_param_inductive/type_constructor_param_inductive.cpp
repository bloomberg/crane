...
uint64_t TypeConstructorParamInductive::size_list(
  ...
  if (std::holds_alternative<typename TypeConstructorParamInductive::wrapped<
          List, uint64_t>::Wrap>(w.v())) {
    const auto &[a0] = std::get<
        typename TypeConstructorParamInductive::wrapped<List, uint64_t>::Wrap>(
        w.v());
    return a0.length();
  } else {
    const auto &[a0, a1] = std::get<
        typename TypeConstructorParamInductive::wrapped<List, uint64_t>::Pair2>(
        w.v());
...
uint64_t TypeConstructorParamInductive::size_opt(
  ...
  if (std::holds_alternative<typename TypeConstructorParamInductive::wrapped<
          std::optional, uint64_t>::Wrap>(w.v())) {
    ...
    const auto &[a0, a1] =
        std::get<typename TypeConstructorParamInductive::wrapped<
            std::optional, uint64_t>::Pair2>(w.v());
    return ([&]() -> uint64_t {
      if (a0.has_value()) {
        const uint64_t &_x = *a0;
        return UINT64_C(1);
