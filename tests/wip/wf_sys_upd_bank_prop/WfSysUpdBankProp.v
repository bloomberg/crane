...
(* Behavioral candidate: updating a bank inside the RAM system preserves system shape. *)

From Stdlib Require Import List Nat Bool.
Import ListNotations.

Module WfSysUpdBankProp.
...
Definition t : Prop :=
  WF init_state /\
  WF_bank empty_bank /\
  length (upd_bank_in_sys init_state 1 empty_bank) = 4 /\
  Forall WF_bank (upd_bank_in_sys init_state 1 empty_bank).

End WfSysUpdBankProp.

Require Crane.Extraction.
From Crane Require Mapping.Std Mapping.NatIntStd.
Crane Extraction "wf_sys_upd_bank_prop" WfSysUpdBankProp.
