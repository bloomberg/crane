...
(** Token labels for the JSON literal lexer. *)
Module Export Labels <: Label.

  (** Variant for each JSON token kind. *)
  ...
  (** Decidable equality on JSON token labels. *)
...
(** Instantiation of the lexer functor for the JSON literal lexer. *)
...
(** Lexer rule matching JSON whitespace (space, tab, newline, carriage return). *)
Definition ru_ws : Rule := (WS, ws_carr_re).


(** Numbers **)
(** Regex for the exponent part of a JSON number (e/E followed by optional sign and digits). *)
Definition exp_part_re := IterApp [(string_union "Ee"); (Optional (string_union "+-")); nat_re].
(** Regex for a JSON floating-point number (decimal with optional exponent). *)
Definition float_re := App dec_re (Optional exp_part_re).

(** Lexer rule matching a JSON floating-point number. *)
Definition ru_float := (FLOAT, float_re).
(** Lexer rule matching a JSON integer. *)
Definition ru_int := (INT, int_re).


(** STRING **)
(** Regex for a single Unicode hex digit (0–9, a–f, A–F). *)
Definition unicode_digit_re := string_union "0123456789aAbBcCdDeEfF".
(** Regex for exactly four Unicode hex digits. *)
Definition four_unicode_digits_re := IterApp [unicode_digit_re ; unicode_digit_re ; unicode_digit_re ; unicode_digit_re].
(** Regex for a Unicode escape sequence (\uXXXX). *)
Definition unicode_codepoint_re := App (string_app "\u") four_unicode_digits_re.
(** Regex for a single valid character inside a JSON string. *)
Definition json_char_re := IterUnion [ unicode_codepoint_re ; AZ_re; az_re; digit_re; ws_re; punc_re; esc_quote_re; esc_bslash_re ].
(** Regex for a complete JSON double-quoted string. *)
Definition json_string_re := IterApp [quote_re; Star json_char_re; quote_re].
(** Lexer rule matching a JSON string literal. *)
Definition ru_string := (STRING, json_string_re).

(** keywords **)
(** Lexer rule matching the JSON keyword [true]. *)
Definition ru_true := (TRUE, string_app "true").
(** Lexer rule matching the JSON keyword [false]. *)
Definition ru_false := (FALSE, string_app "false").
(** Lexer rule matching the JSON keyword [null]. *)
...
(** Ordered list of all JSON lexer rules. *)
