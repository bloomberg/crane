 ...
 **)
(** User-supplied semantic actions for the template lexer, mapping tokens to typed values. *)
Module User <: SemUser STT.

  (** Maps each template token label to its semantic Rocq type. *)
  ...
  (** Parses a raw character list as an unsigned integer, without validating leading zeros. *)
  ...
  (* Doesn't check for leading 0 or -0 *)
  (** Parses a raw character list as a signed integer, without validating leading zeros or -0. *)
  ...
  (* Checks for 0 and -0 *)
  (** Parses a character list as an integer, rejecting -0 and leading zeros. *)
  Definition String2int (z : String) : option Z :=
    match z with
    (* Just a 0 *)
...
(** Instantiation of the semantic lexer functor for the template. *)
