...
public:
    ...
    static bsl::optional<T> with_control(C&& control, F&& f) {
    ...
    T read(const TVar<T>& var) {
        ...
        return *bsl::static_pointer_cast<T>(value);
    }
    ...
    void write(const TVar<T>& var, T value) {
        auto boxed = bsl::static_pointer_cast<void>(
            bsl::make_shared<T>(bsl::move(value)));

        auto ctrl = var.control_block();
        auto it = vars_.find(ctrl);

        if (it != vars_.end()) {
            it->second.write(bsl::move(boxed));
        } else {
            vars_.emplace(ctrl, detail::LogVar::make_write(bsl::move(boxed)));
        }
    }
    ...
    T or_(F1&& first, F2&& second) {
        ...
        try {
            ...
            if (e == StmError::Retry) {
                ...
                auto failed_vars = bsl::move(vars_);
                vars_ = bsl::move(backup_vars);

                try {
                    T result = second(*this);
                    // Merge the failed branch's reads for blocking purposes.
                    combine(bsl::move(failed_vars));
                    return result;
                } catch (StmError e2) {
...
private:
    using VarMap = bsl::map<bsl::shared_ptr<VarControlBlock>,
    ...
    void combine(VarMap other) {
        for (auto& [var, value] : other) {
            auto obsolete = bsl::move(value).obsolete();
            if (obsolete && vars_.find(var) == vars_.end()) {
                vars_.emplace(var, bsl::move(*obsolete));
            }
        }
    }
    ...
    void wait_for_change() {
        ...
        VarMap vars = bsl::move(vars_);
        vars_ = VarMap{};

        bsl::vector<bsl::shared_ptr<VarControlBlock>> reads;

        bool blocking = true;
        for (auto& [var, log_var] : vars) {
            auto read_val = bsl::move(log_var).into_read_value();
            ...
            {
                bsl::shared_lock<bsl::shared_mutex> lock(var->value_mutex_);
    ...
    bool commit() {
        ...
        bsl::vector<bsl::shared_lock<bsl::shared_mutex>> read_locks;
        bsl::vector<bsl::unique_lock<bsl::shared_mutex>> write_locks;
        ...
        bsl::vector<WriteEntry> writes;
        bsl::vector<bsl::shared_ptr<VarControlBlock>> written_vars;

        for (auto& [var, log_var] : vars_) {
            auto& v = log_var.variant();

            if (auto* w = bsl::get_if<detail::Write>(&v)) {
                bsl::unique_lock<bsl::shared_mutex> lock(var->value_mutex_);
                size_t idx = write_locks.size();
                write_locks.push_back(bsl::move(lock));
                writes.push_back({w->value, idx});
                written_vars.push_back(var);

            } else if (auto* row = bsl::get_if<detail::ReadObsoleteWrite>(&v)) {
                bsl::unique_lock<bsl::shared_mutex> lock(var->value_mutex_);
                size_t idx = write_locks.size();
                write_locks.push_back(bsl::move(lock));
            ...
            } else if (auto* rw = bsl::get_if<detail::ReadWrite>(&v)) {
                bsl::unique_lock<bsl::shared_mutex> lock(var->value_mutex_);
            ...
            } else if (bsl::holds_alternative<detail::ReadObsolete>(v)) {
                // Nothing to do — only needed for blocking, not consistency.

            } else if (auto* r = bsl::get_if<detail::Read>(&v)) {
                bsl::shared_lock<bsl::shared_mutex> lock(var->value_mutex_);
                // Check consistency.
                if (var->value_.get() != r->value.get()) {
                    return false;
                }
                read_locks.push_back(bsl::move(lock));
            }
        }
        ...
        // Write values while still holding write locks.
        {
            ...
            for (auto& [var, log_var] : vars_) {
                auto& v = log_var.variant();
                if (bsl::holds_alternative<detail::Write>(v) ||
                    bsl::holds_alternative<detail::ReadObsoleteWrite>(v) ||
                    bsl::holds_alternative<detail::ReadWrite>(v)) {
    ...
    struct TransactionGuard {
        TransactionGuard() {
            if (running()) {
                throw bsl::logic_error("STM: Nested Transaction");
            }
            running() = true;
        }
        ~TransactionGuard() {
