...
// with detached threads + a shared promise/atomic flag. bsl::async is
...
template <typename T>
static bsl::optional<T> run_async(int duration_ms,
                                  bsl::function<T()> f,
                                  bsl::function<void()> g) {
    // Shared state: the result slot and a flag.
    auto result = bsl::make_shared<bsl::optional<T>>();
    auto done = bsl::make_shared<bsl::atomic<bool>>(false);
    auto mtx = bsl::make_shared<bsl::mutex>();
    auto cv = bsl::make_shared<bsl::condition_variable>();

    bsl::thread([=]() {
        T val = f();
        {
            bsl::lock_guard<bsl::mutex> lock(*mtx);
            *result = bsl::move(val);
            done->store(true);
        }
        cv->notify_one();
    ...
    for (int elapsed = 0; elapsed < duration_ms; elapsed += 50) {
        {
            bsl::unique_lock<bsl::mutex> lock(*mtx);
            if (cv->wait_for(lock, bsl::chrono::milliseconds(50),
                             [&] { return done->load(); })) {
                return bsl::move(*result);
            }
        }
    }
...
static bool terminates(int duration_ms, bsl::function<void()> f) {
    // Wrap void->void into int->int for run_async.
    auto wrapped = [f_inner = bsl::move(f)]() -> int { f_inner(); return 0; };
    return run_async<int>(duration_ms, bsl::move(wrapped), [](){}).has_value();
}
...
static bool terminates_async(int duration_ms,
                             ...
                             bsl::function<void()> g) {
    auto wrapped = [f_inner = bsl::move(f)]() -> int { f_inner(); return 0; };
    return run_async<int>(duration_ms, bsl::move(wrapped), bsl::move(g)).has_value();
}
...
#define RUN_TEST(name)                                               \
    do {                                                              \
        tests_run++;                                                  \
        bsl::cout << "  " << #name << " ... " << bsl::flush;         \
        try {                                                         \
            name();                                                   \
            tests_passed++;                                           \
            bsl::cout << "ok" << bsl::endl;                           \
        } catch (const bsl::exception& e) {                           \
            tests_failed++;                                           \
            bsl::cout << "FAILED: " << e.what() << bsl::endl;        \
        } catch (...) {                                               \
            tests_failed++;                                           \
            bsl::cout << "FAILED (unknown exception)" << bsl::endl;  \
        }                                                             \
    } while (0)
...
#define ASSERT_TRUE(x)                                               \
    do {                                                              \
        if (!(x)) {                                                   \
            throw bsl::runtime_error(                                 \
                bsl::string(__FILE__) + ":" +                         \
                bsl::to_string(__LINE__) + ": " #x " is false");     \
        }                                                             \
    } while (0)
...
#define ASSERT_FALSE(x)                                              \
    do {                                                              \
        if ((x)) {                                                    \
            throw bsl::runtime_error(                                 \
                bsl::string(__FILE__) + ":" +                         \
                bsl::to_string(__LINE__) + ": " #x " is true");      \
        }                                                             \
    } while (0)
...
void test_write_obsolete_ignore() {
    auto lv = detail::LogVar::make_write(bsl::make_shared<int>(42));
    auto result = bsl::move(lv).obsolete();
...
void test_control_block_blocked() {
    auto ctrl = bsl::make_shared<detail::ControlBlock>();
    auto raw = ctrl.get();
...
void test_transaction_log_read() {
    ...
    TVar<bsl::vector<int>> var({1, 2, 3, 4});
    auto result = atomically([&](Transaction& tx) {
        return var.read(tx);
    });
    ASSERT_EQ(result, (bsl::vector<int>{1, 2, 3, 4}));
}

/// Port of `write_read` (write then read in same transaction)
void test_transaction_log_write_read() {
    TVar<bsl::vector<int>> var({1, 2});

    auto result = atomically([&](Transaction& tx) {
        var.write(tx, bsl::vector<int>{1, 2, 3, 4});
        return var.read(tx);
    });
    ...
    ASSERT_EQ(var.read_atomic(), (bsl::vector<int>{1, 2, 3, 4}));
}
...
void test_transaction_with_control_abort_on_single_run() {
    ...
    ASSERT_EQ(x, bsl::optional<int>(42));
}
...
void test_threaded() {
    ...
    auto result = run_async<int>(800,
        ...
        [&var]() {
            bsl::this_thread::sleep_for(bsl::chrono::milliseconds(100));
...
void test_read_write_interfere() {
    ...
    auto t = bsl::thread([var]() mutable {
        atomically([&var](Transaction& tx) {
            int x = var.read(tx);
            bsl::this_thread::sleep_for(bsl::chrono::milliseconds(500));
            var.write(tx, x + 10);
            return 0;
        });
    });
...
void test_unwrap_none() {
    bsl::optional<int> x = bsl::nullopt;
    bool caught = false;
    try {
        unwrap_or_retry(x);
    } catch (StmError e) {
...
int main() {
    bsl::cout << "Running crane-stm-bde tests..." << bsl::endl;

    bsl::cout << "\n--- TVar tests ---" << bsl::endl;
    RUN_TEST(test_read_atomic);

    bsl::cout << "\n--- LogVar tests ---" << bsl::endl;
    RUN_TEST(test_write_obsolete_ignore);

    bsl::cout << "\n--- ControlBlock tests ---" << bsl::endl;
    ...
    bsl::cout << "\n==============================" << bsl::endl;
    bsl::cout << tests_run << " tests run, "
              << tests_passed << " passed, "
              << tests_failed << " failed." << bsl::endl;
