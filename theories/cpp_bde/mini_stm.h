...
inline T TVar<T>::read() const {
    BSLS_ASSERT(g_tx && "read() must be called inside atomically()");
    Transaction& tx = *g_tx;
    auto self = bsl::const_pointer_cast<TVarBase>(shared_from_this());
    ...
    bsl::unique_lock<bsl::mutex> lk(m);
    T result = value;
    uint64_t ver = version.load(bsl::memory_order_acquire);
    lk.unlock();
...
inline void TVar<T>::write(T newVal) {
    BSLS_ASSERT(g_tx && "write() must be called inside atomically()");
    Transaction& tx = *g_tx;
    auto self = bsl::const_pointer_cast<TVarBase>(shared_from_this());
    detail::track_keepalive(tx, self);
    ...
    // Stage the new value
    tx.add_or_update_staged(StagedWrite::create(static_cast<TVar<T>*>(self.get()), bsl::move(newVal)));
}
